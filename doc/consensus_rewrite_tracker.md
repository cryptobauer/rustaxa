...
| PBFT manager | `pbft/pbft_manager.hpp`, `pbft/pbft_manager.cpp`, `pbft_manager_shim/*` | 3267 lines | `partial` | Split Rust services behind C++ daemon shell | Highest complexity: state machine, finalization, gossip, threading, storage, DAG, votes, pillar, FinalChain. Rust-enabled pillar-vote sync validation now uses a temporary header overlay plus shim-owned helper that returns explicit shim/planner statuses, calls the Rust pillar-vote bundle planner for deterministic period/block/prevalidation/unique-weight threshold decisions, and inserts only Rust-accepted vote hashes with Rust-planned weights. It no longer calls `PillarChainManager::validatePillarVote` or `addVerifiedPillarVote` on this sync path. |
...
| Route PBFT manager pillar-vote bundle validation through Rust | `partial` | The Rust-enabled sync path now enters a shim-owned helper through a narrow early-return hook. The helper gathers signature and DPoS vote-count facts, calls the Rust bundle planner, returns explicit shim/planner statuses plus the first bad vote hash, and inserts only Rust-accepted votes with Rust-planned weights through a temporary PillarChainManager hook. This removes `validatePillarVote` / `addVerifiedPillarVote` revalidation and DPoS relookup from the sync path while keeping live sidecar insertion visible as temporary manager debt. Direct CXX bridge coverage for `plan_pillar_vote_bundle` lives in `rust_consensus_tests`. Remaining PBFT manager state machine, proposal handling, finalization, and gossip orchestration are still C++ owned. |
