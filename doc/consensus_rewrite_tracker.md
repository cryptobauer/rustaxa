...
- Keep protocol-visible and named external/public C++ contracts stable unless a task explicitly changes them. Internal
  Rust-mode manager APIs, constructors, callbacks, events, and object-returning methods are migration scaffolding and
  may be deleted after their production callers migrate.
- Main-only files referenced from upstream-owned C++ files must be included only behind the master
  `RUSTAXA_ENABLE=1` guard.
...
- Keep only physical tarcap peer/socket mechanics, packet wrapping, send/disconnect execution, and lane scheduling in
  C++. Rust owns consensus packet inspection, admission, routing, queueing, send/gossip selection, effect ordering, and
  exact executor-result validation.
...
- New consensus rewrite APIs use operation-specific application pipelines. Prefer canonical bytes, compact facts,
  exact native session/effect identities, and typed protocol plans/effects over generic shadow ingestion, payload-id
  CXX APIs, or eager C++ object materialization.

## Application-Owned Network Pipeline Direction

The deleted generic shadow-ingress arena and ingestion-success-only API are not target architecture. Each live packet
family enters an operation-specific application pipeline with canonical payload bytes and compact transport facts, and
returns a protocol status or ordered typed effects that the handler actually consumes. Rust owns consensus routing and
effect decisions; tarcap owns only physical peer/transport execution and lane scheduling.

Consensus rewrite slices should assume this future direction:

- Pipeline communication preserves canonical bytes under an exact native session/effect identity. An implementation may
  use private payload references internally, but no generic payload-id arena or storage handle crosses CXX and payload
  references never become consensus identities.
...
The application-owned data plane exposes multiple logical pipelines rather than one monolithic consensus pipeline.
...
The intended PBFT manager end state is a Rust-owned protocol runtime private to `ConsensusApplication`, with no
Rust-mode manager compatibility surface. Native application tasks own long-lived manager state, consensus queues,
command/event admission, and ordered protocol effects. Until deleted, the C++ overlay may execute only named leaves and
must report the exact result before Rust advances the runtime cursor.

Do not include these boundaries in the PBFT manager breakthrough slice:

- Network/tarcap internals: peer transport, packet wrapping, physical fanout, physical peer-known mutation, disconnect
  and report execution, and lane scheduling stay outside the consensus manager migration. Rust owns consensus queueing,
  send/gossip selection, dependency ordering, and typed network effects; tarcap executes them.
...
- Named leaf/public objects: temporary C++ materialization of `PbftBlock`, `PbftVote`, `PeriodData`, `DagBlock`,
  `Transaction`, pillar values, and public API return values may remain only for a named public client or unavoidable
  executor leaf. Internal managers, tests, logging, and convenience are not retention reasons.
- Node process mechanics: the App-owned shell may retain one worker thread, interruptible waits, clock reads,
  startup/shutdown wiring, and key custody. Native `ConsensusApplicationRuntime` owns daemon scheduling, lifecycle
  state, retries, and ordered protocol effects; the shell cannot retain a protocol cursor or manager state.
...
| PBFT vote ingress/progress runtime | `rust/crates/rustaxa-consensus/src/pbft_vote_ingress.rs`, `pbft_vote_admission.rs`, `pbft_vote_event.rs`, `pbft_vote_progress.rs`, `pbft_vote_pipeline.rs`, `pbft_vote_runtime.rs`; `rust/crates/rustaxa-bridge/src/network.rs`, `network_slashing.rs` | `rust-owned` for PBFT application flow | Native services own single/bundle ingress, validation-backed admission, durable vote/progress publication, replay protection, thresholds, reward selection, and typed network/slashing effects. The standalone `pbft_vote_progress.rs` and `verified_votes.rs` bridge modules and the Rust-mode `VoteManager` facade are deleted. Tarcap retains physical effects; the exact slashing leaf receives account facts, signs, and submits the transaction. |
| PBFT vote validation planner | `rust/crates/rustaxa-consensus/src/pbft_vote_validation.rs`, `pbft_thresholds.rs` | `rust-owned` | Native PBFT services own canonical decoding, identity/signature/VRF checks, DPoS weight, replay timing, threshold decisions, and weighted payload authority. The standalone validation and verified-vote CXX handles/modules are deleted; concrete FinalChain account facts cross only for the exact external slashing/execution leaves. |
...
boundary, `doc/consensus_consolidation_plan.md` provides active slice design and sequencing, and
...
The PBFT application-runtime cutover is implemented. Rust mode constructs no `PbftManager` facade, manager shim, or
private C++ compatibility executor. `App` owns one `ConsensusApplication` plus a process-only worker shell; the native
root owns startup recovery, daemon scheduling, lifecycle/state progression, sync continuation, proposal and vote
admission, finalization sequencing, and durable period transition. CXX retains exact monotonic timer/stop, Unix-time,
signing/VRF, tarcap transport, FinalChain account facts, pillar-anchor-state facts, and concrete EVM execution leaves.
Canonical bytes and typed reports cross only those ports; pillar public reads use the client query API and post-ack
events use the observer port without a manager facade. Manager task/action transcripts, legacy materializers, and their
compatibility tests are deleted. Public readers
and ingress continue through `ConsensusQueryApi` and `ConsensusNetworkApi`, and pure-C++ mode retains the untouched
upstream manager. That checkpoint left 14,340 bridge lines, 7,628 shim lines, 277 CXX functions, 217 carriers,
14 handles, six shim directories, and 33 non-test bridge consumers. Six per-domain storage-query wrappers also collapsed
to one read-only compatibility handle so the exact host ports still produced a net handle reduction.

The DAG/transaction/proposer application-runtime cutover is implemented. Rust mode constructs no `DagManager`,
`TransactionManager`, or `DagBlockProposer` facade and contains none of their shim directories. Native
`ConsensusApplication` owns DAG verification/admission/sync/lifecycle, transaction verification/admission/packing and
finalization cleanup, and proposer scheduling/retry state. RPC and GraphQL mutations enter one operation-shaped public
transaction task; public reads, debug views, light-history reads, and stats use `ConsensusQueryApi`; transaction, DAG,
DAG-sync, get-DAG-sync, and periodic gossip routes enter `ConsensusNetworkApi`. App supplies only exact process/timer,
digest-signing/VRF, asynchronous VDF, tarcap transport, concrete EVM/gas, FinalChain fact, and best-effort public-observer
leaves. The legacy proposer command/VDF bridge module, transaction-manager bridge module, manager-shaped CXX
exports/carriers/materializers, App getters/ownership, and `transaction_manager_shim_test` are deleted. Native runtime,
query, submission, network, restart, duplicate, partial-admission, and fake-host-port tests replace facade behavior;
the retained Rust-enabled `network_test` covers application lifecycle, PBFT sync, and a two-node native
transaction-to-DAG-proposal-to-peer-finalization path, while untouched original classes remain selected only by the
all-Rust-disabled pure-C++ build. Native verification and proposal packing select default or Cornus DAG/PBFT gas limits
from the exact proposal period, with Cornus activation inclusive and the node proposal limit retained as a cap. Root
bootstrap and sortition field grouping now belong to documented native constructors; their bridge modules are deleted,
and the retained gas/VDF host carriers are consolidated without weakening effect, period, hash-order, or all-or-none
result validation.

The DAG cut left 12,100 bridge lines, 4,075 shim lines, 211 CXX functions, 170 carriers, 14 opaque
handles, three shim directories, zero granular flags, zero partial-service factories, zero compatibility-constructor
calls, and 28 non-test C++ bridge consumers. Relative to the preceding checkpoint, this removes 2,240 bridge lines,
3,553 shim lines, 66 functions, 47 carriers, three shim directories, five production consumers, three bridge modules,
and one compatibility-only test; handles, flags, partial factories, and compatibility constructors remain unchanged.

The pillar application cutover completes the facade families tracked by `CRW-14` and `CRW-16`. Rust mode constructs no
`PillarChainManager` and contains no pillar-manager shim. Native `ConsensusApplication` and its private pillar service
own startup restoration/readiness, vote admission and duplicate handling, threshold state, block construction/linkage,
finalization persistence/cleanup, lifecycle, and the post-ack observation decision. Pillar packet families use
`ConsensusNetworkApi`; RPC and public readers use the bounded period-indexed `ConsensusQueryApi` pillar-data view. App supplies only
exact signing/VRF, tarcap, FinalChain pillar-anchor facts, and best-effort post-ack observer leaves. The pillar bridge
modules, manager-shaped CXX exports/carriers/materializers, App/Network/RPC ownership, shim directory, and
compatibility-only bridge tests are deleted. Native network tests cover atomic malformed/duplicate bundle preflight,
accepted-only follow-up effects, terminal member outcomes, and later-admission cancellation; application-runtime tests
cover kind-3 best-effort post-ack observation; native and focused bridge query tests cover period-indexed pillar data.
`rust_consensus_tests` passes 14/14, while pure-C++ builds and four focused pillar codec/serialization cases preserve the
untouched original `PillarChainManager` reference route. `CRW-12` and `CRW-15` stay active for remaining
bridge runtime/tests and named executor/public materialization, while `CRW-17` remains active for storage/FinalChain
contraction. `CRW-N01` remains active until every handler-local consensus route is audited and migrated.

The checked surface after the pillar cut is 11,334 bridge lines, 2,859 shim lines, 195 CXX functions, 154 carriers,
14 opaque handles, two shim directories, zero granular flags, zero partial-service factories, zero
compatibility-constructor calls, and 26 non-test C++ bridge consumers. Relative to the DAG checkpoint, this removes 764
bridge lines, 1,216 shim lines, 16 functions, 16 carriers, one shim directory, two production consumers, two bridge
...
`BridgePbftService` embeds that CXX-free owner. The former `rustaxa-bridge/src/proposed_blocks.rs` conversion module was
subsequently deleted by the application-runtime cutover. Cross-domain leader
...
registering the deleted shim. A remaining `CRW-14` family proceeds as soon as its selected subsystem owner is private
under `CRW-12` and its named clients migrate; it does not wait for unrelated `CRW-12` families globally.
...
For its original bounded slice, `CRW-01` selected a PBFT-cluster-only Rust application service. At that time, code
mapping did not find a wider root that would delete more active compatibility surface in the same slice, and collecting
independently exposed services would have created a service locator. The later task-owner `CRW-12` decision supersedes
that PBFT-only target: production now converges on one private `ConsensusApplication` composition root while preserving
the original no-service-locator rule through private services, no accessors, and operation-shaped tasks.
...
requires StateAPI's minted-reward total to equal its native plan, validates the final DPoS contract balance after minted
and fee rewards, persists synchronized account/DPoS snapshots, and owns commit approval, crash recovery, and publication.
The version-one concrete execution/provenance contract and version-three pending-publication marker correlate exact
chain/config identity, generation, plan, period, prior root, payload digests, and prior, post-transaction, post-rewards,
and committed descriptors. Staged rewards are idempotent for the exact request and reject conflicting retries; their
projection catalogs the reward-only minted-supply/yield DPoS rows. Concrete validation treats an absent trie row and
the canonical zero value as equivalent, while preserving tombstones and rejecting every other value mismatch. Period
zero means genesis activation for Aspen part two and Cacti; `u64::MAX` is the explicit disabled sentinel. A configured
...
compatibility constructors, and non-test C++ consumers remain 10/1/0/0/0/17. Startup recovery is one native
application-root operation over the borrowed concrete-EVM port; C++ owns no recovery action loop or duplicate staged
state. StateAPI reports contain only independently observed physical facts, while Rust retains request identity and
...
and a missing account cannot enter the concrete projection. Focused pruning coverage proves staged execution rejects
...
the Rust-only methods are guarded by `RUSTAXA_ENABLE`. The guarded `App` startup change implements the authorized
timestamped-backup/full-resync rebuild policy. No bridge include or Rust-only behavior is added to the pure-C++ path;
the pure-C++ FinalChain parity build and native DPoS suite remain the source-selected reference proof.
...
transaction/DAG gossip consumes native snapshots and typed transport effects. Admission, duplicate/missing-data status,
partial DAG-sync commit behavior, ordering, persistence, and peer/gossip decisions are native; tarcap retains packet
wrapping, physical send/gossip/disconnect work, peer bookkeeping, and lane scheduling. This closes the network dependency
for deletion of the three Rust-mode DAG/transaction/proposer facades, but not the global `CRW-N01` item: any remaining
handler-local consensus routing families must still move before its completion condition is met.

The pillar cut closes the pillar-vote ingress dependency for facade deletion. Single-vote and bundle handlers submit
canonical payloads directly to the application-owned network API; Rust owns structural/signature/schedule validation,
duplicate and weighted-threshold admission, state mutation, and accepted-only known/gossip decisions. Tarcap owns only
peer bookkeeping, wrapping, physical fanout, disconnect execution, and lane scheduling. Pillar bundle-request egress
also remains an operation-shaped network route. `CRW-N01` remains active because this slice did not audit every other
handler-local consensus route against its global completion condition.
...
| --- | --- | ---: | --- | --- | --- |
| DAG graph | `dag/dag.hpp`, `dag/dag.cpp`, `rustaxa-consensus::dag::DagGraph` | 424 legacy lines | `rust-owned` | Native `DagService` under `ConsensusApplication`; legacy C++ graph is reference-only | Rust-enabled production owns graph/index state, storage restoration, ordering, cleanup, and bounded query snapshots privately. `ConsensusQueryApi` returns client DTOs and `ConsensusNetworkApi` consumes canonical payloads; neither exposes graph handles. Rust builds exclude untouched legacy `dag.cpp`, while direct `Dag`/`PivotTree` cases remain pure-C++ reference coverage. |
| DAG manager | `dag/dag_manager.hpp`, `dag/dag_manager.cpp` | 1048 legacy lines; Rust-mode facade deleted | `rust-owned`; no Rust-mode C++ facade | Native `DagService` and `DagTransactionService` under `ConsensusApplication` | Native tasks own DAG verification, admission, storage/graph publication, finalized cleanup, non-finalized sync, restart, and transaction composition. Network handlers use `ConsensusNetworkApi`; public readers use `ConsensusQueryApi`; canonical DAG bytes materialize in C++ only for named public-event formatting. The bridge module, manager exports/carriers/materializers, App getter/ownership, shim directory, and compatibility-only tests are deleted. Pure-C++ builds retain the untouched original manager. |
| DAG proposer | `dag/dag_block_proposer.hpp`, `dag/dag_block_proposer.cpp`, native `consensus_application_runtime.rs` | 576 legacy lines; Rust-mode facade deleted | `rust-owned`; no Rust-mode C++ facade | Native application proposer scheduler with exact host leaf ports | Rust owns eligibility, transaction packing, retry/throttle decisions, VDF job sequencing, tip selection, block construction, signing progression, admission, and gossip planning. App supplies exact process/timer, VRF/signing, asynchronous VDF start/poll/cancel, concrete gas estimation, transport, and observer reports without a manager-shaped worker API. The proposer bridge module, command/materialization carriers, App ownership, shim directory, and facade tests are deleted; pure-C++ builds retain the untouched original proposer. |
...
| PBFT chain | `pbft/pbft_chain.hpp`, `pbft/pbft_chain.cpp` | 259 legacy lines | `rust-owned`; no Rust-mode C++ facade | Private `rustaxa-consensus::pbft_chain::PbftChainService` under `ConsensusApplication` | The CXX-free native owner holds startup restoration, storage lifetime, locking, head transition, membership, and validation. Internal PBFT work uses coherent application-root task DTOs; network/RPC readers use the root-bound client query API. The bridge module, chain exports/materializers, shim, constructors, and compatibility tests are deleted. Rust builds import or compile no `PbftChainOld`; pure-C++ builds retain the untouched original implementation and test. |
...
| PBFT manager | `pbft/pbft_manager.hpp`, `pbft/pbft_manager.cpp`, native `consensus_application_runtime.rs` | 3267 legacy lines; Rust-mode facade/executor deleted | `rust-owned`; no Rust-mode manager facade | Native PBFT application runtime with exact host leaf ports | Rust-enabled builds exclude the untouched upstream manager and contain no `PbftManager` object, shim, or private C++ runtime. `App` owns one `ConsensusApplication` and a process-only shell. Native services own startup recovery, daemon/action state, sync, proposal ranking and reproposal, vote/pillar admission, slashing composition, finalization, dynamic lambda, period transition, PBFT chain, proposals, verified votes, and coherent status. CXX supplies only monotonic/Unix clocks, signing and VRF custody, tarcap execution, FinalChain account and pillar-anchor facts, concrete EVM reports, and post-ack observation. Network and public readers use root network/query APIs; no pillar facade remains. Pure-C++ builds retain the untouched original manager. |
...
78 bridge lines and 11 shim lines while C++ retains only the stoppable polling sleep. Focused native ready/wait and
exact overflow-diagnostic coverage,
...
| Vote manager | `vote_manager/vote_manager.hpp`, `vote_manager.cpp` | 1145 legacy lines | `rust-owned`; no Rust-mode C++ facade | Native PBFT vote state and application-root tasks | Rust-mode construction and every production caller have migrated to the single `ConsensusApplication` root, `ConsensusNetworkApi`, or `ConsensusQueryApi`; the five-file overlay, CMake selection, bridge-only operations/carriers/materializers, and compatibility tests are deleted. Native PBFT tasks own generation, validation, admission, progress, certificate/reward selection, cleanup, persistence, snapshots, and egress planning. Tarcap retains transport, C++ retains secret custody/signing and the typed slashing-transaction insertion leaf, and FinalChain execution retains canonical byte materialization where required. The untouched original manager and direct tests remain pure-C++-reference-only. |
| Pillar chain manager | `pillar_chain/pillar_chain_manager.hpp`, `pillar_chain_manager.cpp` | 427 legacy lines; Rust-mode facade deleted | `rust-owned`; no Rust-mode C++ facade | Private native `PillarChainService` under `ConsensusApplication` | Native state owns restoration/readiness, current/latest-finalized snapshots, validator-count history, vote admission/uniqueness/thresholds, block construction/linkage, sync bundles, finalization persistence/cleanup, lifecycle, and post-ack observation. Pillar network handlers use `ConsensusNetworkApi`; public readers use the period-indexed `ConsensusQueryApi` pillar-data view. C++ implements only exact signing/VRF, tarcap, FinalChain pillar-anchor facts, and observer delivery. The bridge modules, manager exports/carriers/materializers, App ownership, shim directory, and compatibility tests are deleted; pure-C++ builds retain the untouched original manager and pillar codecs. |
...
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp` | 837 legacy lines; Rust-mode facade deleted | `rust-owned`; no Rust-mode C++ facade | Native `TransactionService` and application-root tasks | Native state owns queue payloads/metadata, known and finalized sidecars, gas cache/oracle, packing, storage, restart, admission, recovery, and finalization cleanup. Public mutation callers use the application submission task, public reads/stats use `ConsensusQueryApi`, packet ingress/gossip uses `ConsensusNetworkApi`, and concrete gas estimation is an exact unlocked host leaf with typed identity-checked reports. The bridge module, manager-shaped exports/carriers/materializers, App ownership/getter, shim directory, and compatibility test are deleted. Pure-C++ builds retain the untouched original manager. |
| Gas pricer | `transaction/gas_pricer.hpp`, `transaction/gas_pricer.cpp`, `rustaxa-consensus::gas_pricer` | 171 legacy lines | `rust-owned` | Native gas oracle composed into private transaction service state | Native state owns finalized-history restoration, finalized-block updates, configured floors, percentile bids, and queue-aware pricing. Metrics/RPC/GraphQL consume bounded query DTOs and finalization mutates through application tasks; no Rust-mode manager boundary remains. The untouched original implementation and test remain pure-C++-reference-only. |
| Pillar block/votes | `pillar_chain/pillar_block.hpp`, `pillar_chain/pillar_votes.hpp`, matching `.cpp` files | 627 legacy lines | `rust-owned` in Rust mode | Native Rust domain and private pillar service; legacy C++ types are reference-only | `rustaxa-types::pillar` owns block, vote, validator-count change, block-data, optimized-bundle, RLP/Solidity, and hash shapes. Native services own uniqueness, weighted aggregation, threshold selection, cleanup, inspection, sync, persistence, and bounded query views. Canonical bytes cross only exact signing/tarcap/observer or public-query boundaries; pure-C++ builds retain untouched legacy codecs. |
| Pillar manager | `pillar_chain/pillar_chain_manager.hpp`, `pillar_chain/pillar_chain_manager.cpp` | 629 legacy lines; Rust-mode facade deleted | `rust-owned`; no Rust-mode C++ facade | Private `PillarChainService` under the application root | Application tasks own restoration, admission, finalization, persistence, lifecycle, and observer sequencing; network/query callers use named root APIs. The bridge/shim/materialization family and compatibility tests are deleted. Pure-C++ builds retain the untouched legacy manager and `PillarVotes` implementations. |
...
The coordinated `CRW-12`/`CRW-14`/`CRW-15`/`CRW-16` vertical cut retired the Rust-mode `PbftChain` facade as a
complete family. `ConsensusApplication` remains the sole native owner: PBFT manager work reads finalized context through
the coherent application-root `PbftManagerChainContext` task and finalized membership through a task API, while DAG and vote wiring no longer accepts
or retains a chain facade. Network sync/status readers share the client-oriented consensus query handle, whose progress
view is live-root-backed and whose finalized-membership read is explicitly transport-scoped. RPC and debug clients read
the same progress DTO through `AppBase`. The bridge module, five legacy chain exports and their carriers/materializers,
Rust-mode constructors, shim directory, and compatibility-only tests are deleted together. The untouched original
`PbftChain` and `pbft_chain_test` remain available only in the all-Rust-disabled reference build. Retained boundaries are
the named tarcap transport read, public consensus progress query, and PBFT finalization task leaves; no Rust-mode caller
can construct or retrieve a `PbftChain`. Measured surface falls by 4 bridge lines, 242 shim lines, two CXX functions,
and one shim directory to 20,673 bridge lines, 14,306 shim lines, 353 CXX functions, 291 carriers, 15 handles, and eight
shim directories; compatibility constructors remain zero and non-test C++ consumers remain 38. The Rust fast gate,
Rust-enabled smoke gate, inventory guard and self-test, focused bridge/consensus binaries, native insufficient-certificate
test, and the new Rust-enabled two-node PBFT query/sync cutover test pass. An all-Rust-disabled build of `taraxad`,
`network_test`, and the untouched `pbft_chain_test` also passes, including execution of the legacy chain test. The
aggregate consensus target reaches its existing multi-node pillar coverage but `PillarChainTest.votes_count_changes`
still reproduces `PBFT_REWARD_CURSOR_DURABLE_CERT_MISMATCH`; the focused cutover tests remain green. Python integration
setup is blocked in the current image because `virtualenv`, `pytest`, and the system `python3-venv` support are absent.
The repo-wide `check-static` gate still reports the documented pre-existing cppcheck/style baseline, including findings
in untouched upstream consensus sources; no finding identifies the deleted facade or its replacement boundaries.

The `CRW-12`/`CRW-15`/`CRW-16`/`CRW-N01` vertical cut deleted the Rust-mode `VoteManager` facade and its complete
five-file shim overlay. `App`, `PbftManager`, `Network`, tarcap handlers, RPC/debug/Test RPC, and node statistics no
longer construct, retain, or retrieve a vote manager in Rust mode. PBFT generation, validation, admission, round and
certificate selection, reward handling, cleanup, persistence, and vote snapshots call the single application root;
network single-vote and bundle ingress now consumes canonical vote bytes and admits them directly through root-owned
PBFT and FinalChain siblings instead of emitting a C++ `PbftVote` application effect. Tarcap retains physical transport,
and C++ retains only canonical vote materialization, local signing, and typed slashing-transaction insertion leaves.
Public quorum and verified-vote count reads use the live `ConsensusQueryApi`. The obsolete cancellation export, six
dead vote exports, two carriers, storage/application-effect compatibility tests, Rust-mode constructors, CMake overlay,
and the shim directory are removed together; untouched pure-C++ `VoteManager` remains reference-only. Exact surface is
20,560 bridge lines, 12,430 shim lines, 347 CXX functions, 289 carriers, 15 handles, seven shim directories, zero
granular flags/factories/compatibility constructors, and 36 non-test C++ bridge consumers. The following PBFT-manager
cutover folded local vote admission and own-vote persistence into one atomic application task. Native network admission
and vote behavior are covered in Rust, while
the retired application-effect CXX tests no longer preserve the deleted topology. Independent review identified and
closed two admission hazards before closeout: ordinary votes now resolve FinalChain account facts lazily only when a
slashing proof exists, and a composed bundle infrastructure error cancels every not-yet-admitted member. The focused
The manager-shaped Rust branch of `StateAPITest.slashing` is retired; native slashing and FinalChain tests cover the
Rust path while the original StateAPI transcript remains unchanged in pure-C++ mode.
The obsolete external VoteManager period/round lifecycle result and no-op advance action are also deleted because the
application root already commits that progress. The focused
Rust-enabled `NetworkTest.node_sync2` gate still reports
`PBFT_MANAGER_BLOCK_VALIDATION_DAG_ORDER_MISSING`; an isolated build and run at the slice base `77e4067f2` reproduces
the same 13-vs-3 DAG-count failure, so it remains inherited DAG/PBFT integration debt rather than a vote-cutover
regression.

The application-runtime follow-on completes the PBFT-family `CRW-12`/`CRW-15`/`CRW-16` boundary. The private C++
`ConsensusApplication::Runtime`, manager-shaped bridge modules, action/result carriers, object materializers, and
transcript-only `test_pbft_sync.cpp` and `test_proposed_blocks.cpp` fixtures are deleted. Native startup, daemon
scheduling, state actions, proposal/reproposal ranking,
vote admission and slashing, sync continuation, dynamic-lambda/finalization sequencing, full period transition, and
Ficus pillar processing execute under the root. C++ retains exact process clock/stop, signature/VRF custody, tarcap,
FinalChain account facts, pillar-anchor-state facts, and concrete EVM execution leaves. At that PBFT checkpoint the
pillar facade still read canonical current-pillar bytes from the native owner; the later pillar cut deleted it.
Independent review corrected
Unix-second timestamps, prior-round reproposal,
reward-snapshot freshness, single-winner multi-wallet proposal ranking, transactional gossip gating, and pillar cursor
validation before closeout. The root status export now carries its optional vote diagnostics, eliminating the separate
cold-status CXX call. Rust owns FinalChain index pruning behind one exact root leaf; the retained tarcap DAG transport
sends self-contained transaction payloads in Rust mode so peer-known hints cannot race durable native admission.
`FullNodeTest.sync_five_nodes` covers sync continuation, transaction convergence, and pruning. At that PBFT checkpoint,
the inventory measured 14,340 bridge lines, 7,628 shim lines, 277 CXX functions, 217 carriers, 14 handles, six shim
directories, and 33 non-test C++ consumers; the current exact budget is maintained in the bridge/shim audit.
Final independent review also closed three runtime hazards: periodic transport rejection now preserves counters and retries
without terminating the daemon, account facts are loaded only after a confirmed slashing conflict, and the App-owned
...
The latest `CRW-N01` cut removes C++ scalar decoding and construction for latest status and get-next-votes. One immutable
network identity is derived from the application bootstrap's chain, exact genesis, semantic version, light-node, and
history configuration. Native status ingress strictly decodes and canonicalizes the complete packet, rejects checked
period overflow, validates initial identity/history, and returns exact peer-bookkeeping and sync follow-ups; native
egress builds the complete initial or periodic status payload. Get-next-votes consumes its complete request bytes and
emits ordered, chunked, exact-target complete packet effects. Rust status, DAG-sync, and PBFT-sync adapters depend only
on the physical transport base. The mixed Rust `ISyncPacketHandler` route, compatibility casts, packet-object codecs,
and handler-local request throttles are absent from the feature-on composition, while the untouched legacy hierarchy is
source-selected for pure-C++ mode. The checked surface contracts to 5,479 bridge lines, 1,042 shim lines, 95 CXX
functions, 140 carriers, 10 opaque handles, one shim directory, zero granular flags, zero partial-service factories,
zero compatibility-constructor calls, and 17 non-test consumers. The all-handler audit records one remaining Rust-mode
family: `GetPillarVotesBundlePacketHandler` still decodes the scalar period and pillar-block hash and initiates the
response route in C++. `CRW-N01` therefore remains active for that exact residual.

Closeout validation passes `rewrite-validate-fast`, `rewrite-validate-consensus`, all 1,298 native consensus tests,
both bridge unit tests, all nine focused CXX bridge tests, the six Rust-enabled live network/startup/sync cases, all
seven tarcap concurrency cases, the Rust-enabled and fresh all-Rust-disabled `core_libs` builds, the inventory guard and
self-test, and the storage-boundary guard. Ten of eleven registered CTest entries pass; only the established Go static
...
| Class | Public API groups | Dependencies | Tests | Target |
| --- | --- | --- | --- | --- |
| `PbftChain` | Pure-C++ reference head/hash/size reads, block lookup, update head, and block validation | `DbStorage`, `PbftBlock` in pure-C++ mode only | pure-C++ `pbft_chain_test`; native Rust PBFT service/query tests plus `pbft_manager_test` and full-node coverage | Deleted in Rust mode; internal callers use application-root tasks and canonical DTOs, while transport/RPC readers use the client-oriented consensus query API |
| `ProposedBlocks` | push, mark valid, lookup, presence, cleanup, old-block checks | Native PBFT service and native storage | native proposed-block tests, `rust_consensus_tests`, `pbft_manager_test`; pure-C++ PBFT coverage | Deleted in Rust mode; the native application root owns canonical proposal bytes, validation state, ranking, and cleanup without a C++ facade or object materializer |
| `PeriodDataQueue` | push/pop/clear/size/period/last block/old-data cleanup | Native PBFT, DAG/transaction, vote, FinalChain, and storage services | `rustaxa-consensus` period-data queue tests, `rustaxa-bridge` application tests, `pbft_manager_test`, full-node sync tests | Private application-runtime state owns canonical sync payloads and continuation; C++ exposes only the exact tarcap transport leaf and does not materialize or execute queue actions in Rust mode |
| `PbftManager` | lifecycle, state machine, proposal generation, period/round/step, DPoS counts, sync queue, block validation, gossip, finalization, dynamic lambda | native application siblings plus exact timer/process, signing, tarcap, FinalChain-fact, pillar-anchor-fact, and concrete EVM leaf ports | native runtime/service tests, `rust_consensus_tests`, focused `pbft_manager_test`, `vote_test`, `pillar_chain_test`, full-node tests, Python integration | Deleted as a Rust-mode facade; `ConsensusApplication` owns daemon scheduling, state progression, proposal/vote admission, sync continuation, finalization sequencing, period transition, and lifecycle state, while C++ implements only named leaf ports and client query/network APIs |
...
| Class | Public API groups | Dependencies | Tests | Target |
| --- | --- | --- | --- | --- |
| Transaction queue runtime | insert/erase/order/group/contains/size/purge/known tx/min gas price | Native FinalChain facts and private `TransactionService` state under `ConsensusApplication` | native queue/runtime, admission, packing, restart, duplicate, and query tests; pure-C++ reference queue cases | Private native Rust queue; no standalone C++ production facade or bridge-accessible service handle |
| `TransactionManager` | verify/insert/pack/get/finalize status/non-finalized recovery/gas estimation | Native transaction/DAG/FinalChain services plus exact external gas executor | native transaction/submission/query/network tests, `rust_consensus_tests`, RPC/GraphQL/full-node coverage, and pure-C++ `transaction_test` | Deleted in Rust mode. Application tasks own admission, packing, DAG persistence, recovery, finalization cleanup, events, and gas-report validation; `ConsensusQueryApi` owns reads/stats, `ConsensusNetworkApi` owns packet ingress/gossip, and C++ retains only public formatting/event delivery and concrete EVM gas execution. |
| `GasPricer` | Pure-C++ reference gas price reads/calculation only; Rust mode uses native transaction status and finalization tasks | Native transaction runtime; legacy storage/transaction manager only in pure-C++ reference mode | native gas-pricer/runtime tests and transaction/full-node integration; pure-C++-only `gas_pricer_test` | Deleted Rust-mode facade and flag; native transaction service owns the oracle and operation-shaped query/finalization callers use application APIs. |

### Pillar, Rewards, Slashing

| Class | Public API groups | Dependencies | Tests | Target |
| --- | --- | --- | --- | --- |
| `PillarBlock` / `PillarBlockData` | RLP, hash, Solidity encode/decode, validator vote-count deltas, bounded public views | native hashes/storage plus FinalChain pillar-anchor facts | native codec/query/finalization tests; pure-C++ `pillar_chain_test` encoding cases | Native domain and private application state in Rust mode; public views use `ConsensusQueryApi`, and legacy C++ codecs are reference-only |
| `PillarVotes` | vote uniqueness, threshold accumulation, above-threshold selection, cleanup, sync bundles | Native `PillarChainService` and `ConsensusNetworkApi` | native pillar-vote/service/network persistence, restart, duplicate, threshold, and finalization tests; focused `rust_consensus_tests`; pure-C++ pillar cases | Private pillar state owns admission, aggregation, selection, retention, persistence, cleanup, and sync; C++ retains only exact signing, tarcap, and post-ack observer execution |
| `PillarChainManager` | Pure-C++ reference create/validate/generate/finalize/current state API | legacy dependencies only in all-Rust-disabled mode | native application/query/network/observer tests and Rust-enabled integration; pure-C++ `pillar_chain_test` | Deleted in Rust mode; application tasks own lifecycle/finalization/persistence, query/network APIs own client routes, and no compatibility manager or pillar object sidecar survives |
...
| `SlashingManager` | double-voting proof submission | native `SlashingProofService`, `FinalChain`, `TransactionManager` | Rust slashing service/planner tests and native FinalChain slashing execution/restart cases; pure-C++ `StateAPITest.slashing` preserves the legacy manager transcript | Native Rust owns deterministic planning, evidence normalization, duplicate protection, and locking; C++ retains classified account/gas lookup, transaction construction/signing/insertion, and logging executor boundaries |
...
| PBFT chain/proposed-block/queue routing | Rust validation plus `rust_consensus_tests`, native proposed-block/PBFT service/query tests, relevant `pbft_manager_test` cases, and pure-C++ `pbft_chain_test`; no Rust-mode PBFT-chain facade or facade test remains |
...
| Transaction queue behavior | Native Rust queue/runtime, admission, packing, public-submission, duplicate/restart, query, and network validation; `rust_consensus_tests` retains focused ABI/effect coverage; pure-C++ queue-focused `transaction_test` and `gas_pricer_test` preserve legacy reference behavior; no Rust-mode transaction-manager facade test remains |
| Slashing proof planning | Rust byte-level proof-hash/calldata fixtures plus native FinalChain slashing execution and restart coverage; `StateAPITest.slashing` is pure-C++-only because its direct manager-shaped vote-generation/admission transcript was retired with the PBFT bridge surface |
| Pillar vote aggregation and sync bundle validation | Native pillar-service/network tests plus `rust_consensus_tests`, affected Rust-enabled network/full-node coverage, and pure-C++ pillar codec cases; no Rust-mode pillar-manager facade test remains |
| Pillar finalization/query/observation | Native application persistence/restart, duplicate, threshold, finalization, query, and post-ack observer tests plus affected RPC/full-node coverage and pure-C++ `pillar_chain_test` reference cases |
| Pillar/reward behavior | Rust validation plus native pillar/reward tests, affected final-chain/full-node paths, and pure-C++ `pillar_chain_test`/`rewards_stats_test` where legacy parity applies |
