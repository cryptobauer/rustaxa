...
| `dag_manager_shim` | Standalone Rust DAG manager runtime facade behind the C++ `DagManager` API; feature-on builds import and compile no legacy manager scaffold; proposer observations/revalidation are runtime-owned | App/consensus code, DAG tests | C++ public compatibility facade | Remove remaining public DAG-block/transaction materialization when callers can use the Rust runtime or a thinner public facade. |
