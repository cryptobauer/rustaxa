...
- Slice 7 — CXX carrier minimization is active: the transaction finalized-check identity input moved from generated
  ...
  each service composition item rather than a no-caller export sweep.
...
| `CRW-03` | `ready` | Absorb PBFT-private state handles into the PBFT application service, starting with proposed blocks and verified votes, then rewards/sortition state used only by PBFT progress and finalization. | `CRW-02` | Chain-private state is no longer independently located or passed through CXX; obsolete `BridgeProposedBlocks`, `BridgeVerifiedVotes`, and PBFT-only rewards/sortition exports and shim mechanics are deleted after their callers migrate. |
...
| PBFT manager | `pbft/pbft_manager.hpp`, `pbft/pbft_manager.cpp`, `pbft_manager_shim/*` | 3267 lines | `partial` | PBFT application service plus C++ lifecycle/executor facade | Rust-enabled builds exclude the untouched upstream manager source and expose only the shim-owned `PbftManager` facade. `BridgePbftService` owns manager scalar state, daemon/action/session cursors, period-data metadata, sync admission, proposal planning, block validation, transition persistence, and PBFT chain lifetime/state. Production construction restores chain state first, derives manager period/Cacti activation, and remains bootstrap-gated until C++ replay/restart work completes. Finalization drains and validates the chain mutation internally; C++ executes only the remaining typed external effects for FinalChain/EVM, DAG, network, timers, signing, events, and compatibility materialization. Proposed-block and verified-vote sidecars remain the next private-state absorption target under `CRW-03`. |
