...
| `rust/crates/rustaxa-bridge/src/pillar_votes.rs` | `BridgePillarVotes`, `create_pillar_votes_index` | `pillar_votes_shim`, period-data/vote paths | C++ public compatibility facade | Delete after pillar vote indexing/admission is moved into Rust pillar/PBFT runtime and no C++ index facade remains. No-caller reference/snapshot CXX exports are deleted; C++ uses retained payload lookup. |
