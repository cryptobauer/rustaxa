...
| `rust/crates/rustaxa-bridge/src/network.rs` | One `Network`-owned operation-specific effect root with transport-lane-partitioned queues, effect-ID-correlated vote admission, native PBFT-sync response construction, accepted-only bundle aggregation, dependency-checked publication/gossip, and no generic shadow-ingress arena | latest/v5 tarcap handler families | External boundary | Complete `CRW-N01`; retain transport-only execution API while the remaining admission and sync-intake leaves migrate. |
