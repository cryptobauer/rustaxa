...
| `rust/crates/rustaxa-bridge/src/transaction_manager.rs` | DTO conversion plus short-lived mutation/admission/cache-store/queue-finalization/sidecar/packing adapters over native transaction ownership; read tasks call lock-owning native services directly | transaction/DAG/PBFT shims | Internal bridge route | Move the remaining mutating FFI-shaped task methods into native service APIs; retain only submission/materialization conversion and unlocked EVM leaf adapters. |
