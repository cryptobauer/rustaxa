...
| `rust/crates/rustaxa-bridge/src/dag.rs` | `BridgeDagManagerRuntime` | `dag_manager_shim`, DAG manager/proposer tests | C++ public compatibility facade | Native `DagManagerState` owns both DAG graphs, proposer observations, and keyed proposal construction/signing intent state. C++ supplies only external facts, VDF RLP, and signature bytes before executing the returned signed-block RLP; keep narrowing the manager until the C++ facade becomes a thin external executor API. |
...
| `dag_manager_shim` | Standalone Rust DAG manager runtime facade behind the C++ `DagManager` API; feature-on builds import and compile no legacy manager scaffold; proposer observation, construction, unsigned-intent, and signed-RLP state are runtime-owned | App/consensus code, DAG tests | C++ public compatibility facade | Remove remaining public DAG-block/transaction materialization when callers can use the Rust runtime or a thinner public facade. |
