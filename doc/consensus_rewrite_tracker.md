...
| Transaction queue | `transaction/transaction_queue.hpp`, `transaction/transaction_queue.cpp`, `transaction_queue_shim/*` | 501 lines | `rust-backed` | Rust metadata domain behind C++ overlay shim | Deterministic queue metadata, per-account nonce ordering, same-nonce replacement, non-proposer expiry planning, pool limits, and gas-price threshold accounting route through Rust under `RUSTAXA_ENABLE_TRANSACTION_QUEUE`. C++ keeps live transaction pointers, known-cache expiration, overflow wall-clock state, and FinalChain account reads for purge. |
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp`, `transaction_manager_shim/*` | 837 lines | `partial` | C++ live sidecar plus Rust packing/persistence/finalized-status/storage-read planner | `packTrxs` candidate sizing, declared-gas fit checks, invalid-estimate demotion decisions, accepted gas accounting, and stop decisions route through `rustaxa-consensus::transaction_manager` under `RUSTAXA_ENABLE`. `saveTransactionsFromDagBlock` now sends live transaction facts to Rust; Rust owns duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, target `TrxCount` planning, and the accepted transaction storage batch before C++ updates only accepted live non-finalized/pool sidecars. `getTransaction`, `getTransactions`, and `getBlockTransactions` now check live C++ views first, then ask Rust storage to resolve pending, finalized regular, finalized system, or missing transaction payloads before C++ materializes objects and applies proposal-period nonce filtering for finalized storage hits. `recoverNonfinalizedTransactions` now asks Rust storage for hash/RLP recovery entries and stale-finalized classification; Rust removes stale pending rows before C++ validates hash/RLP consistency and rebuilds only survivor sidecars. `updateFinalizedTransactionsStatus` now sends finalized hashes and live non-finalized-cache facts to Rust; Rust plans count increments, stale recently-finalized period eviction, and periodic queue purge while persisting `TrxCount` before C++ mutates live maps/cache/pool sidecars. C++ still owns live transactions, locks, gas estimation/cache, queue mutation, FinalChain account fact reads, and broader lifecycle/finalization state. |
...
| Class | Public API groups | Dependencies | Tests | Target |
| --- | --- | --- | --- | --- |
| `TransactionQueue` | insert/erase/get/order/group/contains/size/purge/known tx/min gas price | `FinalChain`, transactions | `transaction_test`, `transaction_queue_shim_test`, `gas_pricer_test`, `full_node_test` pool cases | Rust metadata queue behind a full overlay shim; C++ keeps live transactions and FinalChain purge reads |
| `TransactionManager` | verify/insert/pack/get/finalize status/non-finalized recovery/gas estimation | `DbStorage`, `FinalChain`, thread pool, `DagBlock`, state API | `transaction_test`, `transaction_manager_shim_test`, `dag_block_test`, `pbft_manager_test`, `full_node_test` | Rust-backed packing, DAG persistence, finalized-status planning, storage-backed transaction lookup, non-finalized recovery reads, finalized filter/verification helpers, verification/validated-insert admission planning, and Rust-mode pending-transaction event emission with C++ live state/materialization/estimation shell; live pool/non-finalized/count read helpers are shim-owned; continue moving live queue ownership and lifecycle mutation before broader orchestration |
