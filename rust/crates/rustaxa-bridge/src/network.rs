...
impl BridgeConsensusNetworkApi {
    ...
    pub fn consensus_network_queue_pbft_sync_egress_request_effects(
        &self,
        effects: rustaxa_ffi::NetworkPbftSyncEgressRequestEffects,
    ) -> anyhow::Result<rustaxa_ffi::NetworkIngressDecision> {
        let mut api = self
            .api
            .lock()
            .map_err(|_| anyhow::anyhow!("consensus network api lock poisoned"))?;
        Ok(to_bridge_network_ingress_decision(
            api.queue_pbft_sync_egress_request_effects(to_domain_pbft_sync_egress_request_effects(
                effects,
            )),
        ))
    }
    ...
    pub fn consensus_network_queue_pillar_votes_bundle_egress_request_effects(
        &self,
        effects: rustaxa_ffi::NetworkPillarVotesBundleEgressRequestEffects,
    ) -> anyhow::Result<rustaxa_ffi::NetworkIngressDecision> {
        let mut api = self
            .api
            .lock()
            .map_err(|_| anyhow::anyhow!("consensus network api lock poisoned"))?;
        Ok(to_bridge_network_ingress_decision(
            api.queue_pillar_votes_bundle_egress_request_effects(
                to_domain_pillar_votes_bundle_egress_request_effects(effects),
            ),
        ))
    }
    ...
    pub fn consensus_network_queue_dag_sync_egress_request_effects(
        &self,
        effects: rustaxa_ffi::NetworkDagSyncEgressRequestEffects,
    ) -> anyhow::Result<rustaxa_ffi::NetworkIngressDecision> {
        let mut api = self
            .api
            .lock()
            .map_err(|_| anyhow::anyhow!("consensus network api lock poisoned"))?;
        Ok(to_bridge_network_ingress_decision(
            api.queue_dag_sync_egress_request_effects(to_domain_dag_sync_egress_request_effects(
                effects,
            )),
        ))
    }
    ...
    pub fn consensus_network_queue_pillar_vote_admission_request_effects(
        &self,
        effects: rustaxa_ffi::NetworkPillarVoteAdmissionRequestEffects,
    ) -> anyhow::Result<rustaxa_ffi::NetworkIngressDecision> {
        let mut api = self
            .api
            .lock()
            .map_err(|_| anyhow::anyhow!("consensus network api lock poisoned"))?;
        Ok(to_bridge_network_ingress_decision(
            api.queue_pillar_vote_admission_request_effects(
                to_domain_pillar_vote_admission_request_effects(effects),
            ),
        ))
    }
    ...
    pub fn consensus_network_queue_pillar_vote_bundle_member_admission_request_effects(
        ...
        let mut api = self
            ...
            .map_err(|_| anyhow::anyhow!("consensus network api lock poisoned"))?;
        Ok(to_bridge_network_ingress_decision(
            api.queue_pillar_vote_bundle_member_admission_request_effects(
                to_domain_pillar_vote_admission_request_effects(effects),
            ),
        ))
    }
...
fn to_domain_pbft_sync_egress_request_effects(
    value: rustaxa_ffi::NetworkPbftSyncEgressRequestEffects,
) -> rustaxa_consensus::NetworkPbftSyncEgressRequestEffects {
    rustaxa_consensus::NetworkPbftSyncEgressRequestEffects {
        peer_id: value.peer_id,
        from_period: value.from_period,
        blocks_to_transfer: value.blocks_to_transfer,
        pbft_chain_synced: value.pbft_chain_synced,
        source_payload_id: value.source_payload_id,
        request_sync: value.request_sync,
    }
}

fn to_domain_pillar_votes_bundle_egress_request_effects(
    value: rustaxa_ffi::NetworkPillarVotesBundleEgressRequestEffects,
) -> rustaxa_consensus::NetworkPillarVotesBundleEgressRequestEffects {
    rustaxa_consensus::NetworkPillarVotesBundleEgressRequestEffects {
        peer_id: value.peer_id,
        period: value.period,
        pillar_block_hash: value.pillar_block_hash,
        source_payload_id: value.source_payload_id,
        request_bundle: value.request_bundle,
    }
}

fn to_domain_dag_sync_egress_request_effects(
    value: rustaxa_ffi::NetworkDagSyncEgressRequestEffects,
) -> rustaxa_consensus::NetworkDagSyncEgressRequestEffects {
    rustaxa_consensus::NetworkDagSyncEgressRequestEffects {
        peer_id: value.peer_id,
        peer_period: value.peer_period,
        requested_block_hashes: value
...
fn to_domain_pillar_vote_admission_request_effects(
    value: rustaxa_ffi::NetworkPillarVoteAdmissionRequestEffects,
) -> rustaxa_consensus::NetworkPillarVoteAdmissionRequestEffects {
    rustaxa_consensus::NetworkPillarVoteAdmissionRequestEffects {
        peer_id: value.peer_id,
        vote_hash: value.vote_hash,
        period: value.period,
