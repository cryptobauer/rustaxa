...
mod hash_to_prime_boundary_tests {
        ...
        for lambda in 0..=5 {
            ...
            if let Ok(prime) = result {
                ...
                assert_eq!(prime.is_probably_prime(10), IsPrime::Yes,
                    "Result should be prime for lambda={}", lambda);
            }
        }
    }
    ...
    fn test_very_large_input() {
        ...
        assert!(result.is_ok(), "Hash-to-prime should handle very large input");
    ...
    fn test_max_iterations_boundary() {
        ...
        for i in 0..10 {
            ...
            match result {
                ...
                Err(msg) => {
                    assert!(msg.contains("Prime not found within"),
    ...
    fn test_deterministic_across_boundaries() {
        ...
        for (lambda, input) in test_cases {
            ...
            assert_eq!(result1.is_ok(), result2.is_ok(),
                "Consistency check failed for lambda={}, input={}", lambda, input);

            if result1.is_ok() && result2.is_ok() {
                assert_eq!(result1.unwrap(), result2.unwrap(),
                    "Determinism check failed for lambda={}, input={}", lambda, input);
            }
        }
    }
    ...
    fn test_hash_to_prime_with_boundary_inputs() {
        ...
        for input in boundary_inputs {
            ...
            if let Ok(prime) = result {
                use rug::integer::IsPrime;
                assert_eq!(prime.is_probably_prime(10), IsPrime::Yes,
                    "Result should be prime for input: {}", input);
...
mod stress_and_edge_tests {
    ...
    fn test_hash_to_prime_collision_resistance() {
        ...
        for i in 0..100 { // Reduced from 1000 to 100 to keep test fast
            ...
            if let Ok(prime) = h2p.hash_to_prime(&input) {
                ...
                assert!(!seen_primes.contains(&prime),
                    "Collision detected: prime {} generated for inputs with different values", prime);
                seen_primes.insert(prime);
            }
        }

        assert!(seen_primes.len() > 95, "Too many hash-to-prime failures: only {} successes", seen_primes.len());
    }
    ...
    fn test_memory_usage_with_large_numbers() {
        ...
        for (i, input) in large_inputs.iter().enumerate() {
            let result = h2p.hash_to_prime(input);
            assert!(result.is_ok(), "Failed for large input #{}: bits={}", i, input.significant_bits());
            ...
            if let Ok(prime) = result {
                ...
                assert!(prime.significant_bits() < 1000,
                    "Prime too large: {} bits for input #{}", prime.significant_bits(), i);
            }
        }
    }
    ...
    fn test_precision_bound_caching() {
        ...
        assert!(duration2 < duration1, "Second call should be faster than first");
        assert!(duration3 < duration1, "Third call should be faster than first");
    }
}
