...
/// is sufficient. Consensus and code failures retain the exact error string
/// supplied by the execution boundary and terminate estimation.
...
/// successful probe records its `gas_used`; a code failure lowers the known
/// bound, while a consensus failure terminates immediately with its exact
/// string. Callback errors are propagated as [`EstimateError::Callback`].
///
...
/// a code failure returns the smallest currently known successful bound rather
/// than repeatedly probing the same gas limit.
    ...
    F: FnMut(u64) -> Result<EstimateProbe, E>,
{
    ...
    while high - low > high / 20 {
        ...
        match probe(midpoint).map_err(EstimateError::Callback)? {
            ...
            EstimateProbe::CodeFailure { .. } if midpoint == low => return Ok(high),
