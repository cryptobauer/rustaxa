...
#define RUSTAXA_PBFT_VOTE_NETWORK_ARGS consensus_network_api, version,
#else
#define RUSTAXA_LEGACY_DB_ARG db,
#define RUSTAXA_NETWORK_API_ARG
#define RUSTAXA_PBFT_VOTE_NETWORK_ARGS
#endif
...
const TaraxaCapability::InitPacketsHandlers TaraxaCapability::kInitLatestVersionHandlers =
    [](const std::string &logs_prefix, const FullNodeConfig &config, const h256 &genesis_hash,
      ...
      packets_handlers->registerHandler<VotePacketHandler>(config, peers_state, packets_stats,
#ifndef RUSTAXA_ENABLE
                                                           pbft_mgr, pbft_chain, vote_mgr, slashing_manager,
#else
                                                           consensus_status, pbft_chain,
#endif
                                                           RUSTAXA_PBFT_VOTE_NETWORK_ARGS node_addr, logs_prefix);
#ifndef RUSTAXA_ENABLE
...
#endif
      packets_handlers->registerHandler<VotesBundlePacketHandler>(config, peers_state, packets_stats,
#ifndef RUSTAXA_ENABLE
                                                                  pbft_mgr, pbft_chain, vote_mgr, slashing_manager,
#else
                                                                  consensus_status, pbft_chain,
#endif
                                                                  RUSTAXA_PBFT_VOTE_NETWORK_ARGS node_addr,
                                                                  logs_prefix);
  ...
  // Standard packets with mid processing priority
      ...
      packets_handlers->registerHandler<DagBlockPacketHandler>(config, peers_state, packets_stats, pbft_syncing_state,
                                                               ...
                                                               logs_prefix);
#else
      packets_handlers->registerHandler<RustDagBlockPacketHandler>(
          config, peers_state, packets_stats, pbft_syncing_state, pbft_chain, consensus_status, consensus_network_api,
          version, node_addr, logs_prefix);
#endif
      ...
      packets_handlers->registerHandler<DagSyncPacketHandler>(config, peers_state, packets_stats, pbft_syncing_state,
                                                              ...
                                                              logs_prefix);
#else
      packets_handlers->registerHandler<RustDagSyncPacketHandler>(
          config, peers_state, packets_stats, pbft_syncing_state, pbft_chain, consensus_status, consensus_network_api,
          version, node_addr, logs_prefix);
#endif
      ...
      packets_handlers->registerHandler<PbftSyncPacketHandler>(config, peers_state, packets_stats, pbft_syncing_state,
                                                               ...
                                                               logs_prefix);
#else
      packets_handlers->registerHandler<RustPbftSyncPacketHandler>(
          config, peers_state, packets_stats, pbft_syncing_state, pbft_chain, consensus_status, final_chain,
          consensus_network_api, node_addr, logs_prefix);
#endif
      packets_handlers->registerHandler<PillarVotePacketHandler>(config, peers_state, packets_stats,
#ifndef RUSTAXA_ENABLE
                                                                 pillar_chain_mgr,
#else
...
#ifdef RUSTAXA_ENABLE
      packets_handlers->registerHandler<RustPbftBlocksBundlePacketHandler>(config, peers_state, packets_stats,
                                                                           consensus_network_api, final_chain,
...
#undef RUSTAXA_PBFT_VOTE_NETWORK_ARGS
