...
10. Port transaction queue behavior before transaction manager orchestration. The Rust-mode `TransactionQueue` overlay
   ...
   zero-stake, zero-total-DPoS, and zero-weight outcomes as stable statuses. The shim currently uses this as a
   production parity oracle while still returning the temporary `VoteManagerOld`/`PbftVote` sidecar; the next routing
   step is direct sidecar materialization from Rust-generated bytes after integration coverage proves add/persist/gossip
   flows.
