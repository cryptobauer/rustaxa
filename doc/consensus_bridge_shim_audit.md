...
| `rust/crates/rustaxa-bridge/src/sortition.rs` | Sortition methods on `BridgeDagTransactionService` | `sortition_params_manager_shim`, DAG/proposer, and the PBFT cross-service finalization operation | Application-service state plus C++ compatibility facade | The standalone handle and factories are deleted. PBFT finalization now validates its cursor and retained storage-stage change before atomically publishing sortition state through the two application-owned services; the C++ commit/report bounce is deleted. Keep the facade methods for public compatibility, DAG verification, and proposer callers until their remaining fact relays move native. |
...
| `sortition_params_manager_shim` | Service-backed sortition compatibility facade with shim-owned compatibility carrier/codec and no independent Rust handle | DAG/sortition compatibility paths | C++ public compatibility facade | `SortitionParamsManagerOld`, the standalone Rust handle/factories, the redundant module flag, and the PBFT-finalization commit helper are retired. Master Rust mode injects the canonical DAG/transaction service; PBFT finalization accesses that owner only through a narrow shared-service accessor and the Rust cross-service operation. Delete after DAG/proposer use native service operations without the facade. |
