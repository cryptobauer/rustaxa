...
| `TransactionManager` | verify/insert/pack/get/finalize status/non-finalized recovery/gas estimation | `DbStorage`, `FinalChain`, thread pool, `DagBlock`, state API | `transaction_test`, `transaction_manager_shim_test`, `dag_block_test`, `pbft_manager_test`, `full_node_test` | Rust-backed `packTrxs` planner with C++ live state/estimation shell; continue moving deterministic helpers before broader lifecycle state |
...
storage for finalized-block counter facts, expired block hashes, and transaction cleanup hashes before C++ applies local
cache and transaction-manager side effects. `DagManager::setDagBlockOrder()` now uses a single Rust apply call that
resolves the anchor level from Rust storage, computes finalization on a candidate state, applies finalized-block counter
updates and expired DAG deletes in one Rust storage batch, commits Rust state, and returns only the remaining shim-side
cleanup facts. Rust-mode non-finalized DAG sync reads the
...
cache cleanup and other transaction-manager side effects.
