...
| `rust/crates/rustaxa-bridge/src/pbft_manager.rs` | Thin `BridgePbftService` adapter plus manager DTO/effect adapters | App, PBFT/vote/pillar shims | Native service wrapper | Native `PbftService` owns coherent sibling restoration, composition, bootstrap readiness, finalization start/resume, all typed executor advancement, owned-action draining, terminal cleanup, and lock-coherent snapshots; the bridge has no manager, chain, proposed-block, verified-vote, slashing, or pillar accessor. Retain compact FFI/error-mapping/external-leaf tests while contracting the remaining C++ executor leaves. |
| `rust/crates/rustaxa-bridge/src/pbft_sync.rs` | CXX conversion over native sync admission/egress tasks plus cert-vote bundle validation conversion | PBFT manager and tarcap | Internal bridge route | Native `PbftService` owns admission cursor lifecycle, report validation, terminal cleanup, bootstrap gating, and storage-backed egress; delete the conversion surface with the PBFT/network pipeline clients. |
...
| `rust/crates/rustaxa-bridge/src/proposed_blocks.rs` | PBFT DTO adapters over root `PbftService` proposal tasks plus stateless storage and temporary-candidate compatibility helpers | PBFT manager, storage, and vote shims | Internal bridge route | Native state, storage, restoration, lock ownership, and standalone behavior live in `rustaxa-consensus`; production calls cannot borrow the proposal sibling, the C++ proposed-block facade is deleted, and the remaining three storage/vote helpers retire with their named compatibility clients. |
...
| `rust/crates/rustaxa-bridge/src/verified_votes.rs` | DTO/effect adapters over task-shaped native `PbftService` methods | vote and PBFT adapters | Internal bridge route | Native storage lifetime, restoration, vote-runtime lock ownership, replay/query/cleanup tasks, own-vote and progress persistence, optimized-bundle planning/building, coherent snapshots, FinalChain-composed validation/admission/threshold tasks, reward finalization, leader selection, and behavioral tests live in `rustaxa-consensus`; retain only carrier conversion at named executor boundaries. |
