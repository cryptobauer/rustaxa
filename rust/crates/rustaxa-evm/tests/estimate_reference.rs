#[path = "../src/estimate.rs"]
mod estimate;

use estimate::{EstimateError, EstimateProbe, estimate_gas};

fn threshold_probe(threshold: u64, transcript: &mut Vec<u64>) -> impl FnMut(u64) -> Result<EstimateProbe, ()> + '_ {
    move |gas| {
        transcript.push(gas);
        Ok(if gas >= threshold {
            EstimateProbe::Success { gas_used: threshold }
        } else {
...
fn handles_zero_and_one_unit_caps_without_nonprogress() {
    ...
    assert_eq!(estimate_gas(1, threshold_probe(2, &mut one_failure)), Err(EstimateError::CodeFailure {
        error: "execution reverted".into(),
    }));
    assert_eq!(one_failure, vec![1]);
}
...
fn supports_maximum_cap_without_arithmetic_overflow() {
    ...
    assert_eq!(transcript[0], u64::MAX);
}

#[test]
fn initial_failures_preserve_typed_kind_and_exact_message() {
    let consensus = estimate_gas(100, |_| Ok::<EstimateProbe, ()>(EstimateProbe::ConsensusFailure {
        error: "nonce too low".into(),
    }));
    assert_eq!(consensus, Err(EstimateError::ConsensusFailure { error: "nonce too low".into() }));

    let code = estimate_gas(100, |_| Ok::<EstimateProbe, ()>(EstimateProbe::CodeFailure {
        error: "execution reverted: denied".into(),
    }));
    assert_eq!(code, Err(EstimateError::CodeFailure { error: "execution reverted: denied".into() }));
}
...
fn midpoint_consensus_failure_is_terminal_but_code_failure_lowers_bound() {
    ...
    let consensus = estimate_gas(100, |gas| {
        ...
        Ok::<EstimateProbe, ()>(if gas == 70 {
            EstimateProbe::ConsensusFailure { error: "future block".into() }
        } else {
            EstimateProbe::Success { gas_used: 40 }
        })
    });
    assert_eq!(consensus, Err(EstimateError::ConsensusFailure { error: "future block".into() }));
    ...
    let code = estimate_gas(100, |gas| {
        ...
        Ok::<EstimateProbe, ()>(if gas < 60 {
            EstimateProbe::CodeFailure { error: "out of gas".into() }
        } else {
