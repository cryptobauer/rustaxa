...
RustaxaNetworkShim::RustaxaNetworkShim(size_t queue_size)
    : packet_arena_(rustaxa::create_packet_arena(1024)), network_(rustaxa::create_network(*packet_arena_, queue_size)) {
  network_->start_network();
}

bool RustaxaNetworkShim::queueIsFull() { return network_->queue_is_full(); }

void RustaxaNetworkShim::ingestPacket(SubprotocolPacketType packet_type, const dev::p2p::NodeID& node_id,
                                      const dev::RLP& rlp) {
  network_->ingest_network_packet(static_cast<uint8_t>(packet_type), node_id.asArray(),
