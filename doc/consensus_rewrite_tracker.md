...
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp`, `transaction_manager_shim/*` | 837 lines | `partial` | Rust-owned live sidecars plus C++ materialization/orchestration shell | `packTrxs` candidate sizing, declared-gas fit checks, invalid-estimate demotion decisions, accepted gas accounting, and stop decisions route through `rustaxa-consensus::transaction_manager` under `RUSTAXA_ENABLE`; invalid-estimate demotion now mutates Rust queue state directly instead of erase/reinsert through C++. The Rust-mode shim owns an opaque Rust sidecar handle for live non-finalized and recently-finalized transaction membership plus canonical RLP payloads. `saveTransactionsFromDagBlock` sends transaction/account facts to Rust; Rust owns sidecar membership checks, duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, target `TrxCount` planning, the accepted transaction storage batch, and accepted non-finalized sidecar insertion before C++ updates only pool side effects. `getTransaction`, `getTransactions`, and `getBlockTransactions` now check the live C++ pool and Rust sidecar payloads first, then ask Rust storage to resolve pending, finalized regular, finalized system, or missing transaction payloads before C++ materializes objects and applies proposal-period nonce filtering for finalized storage hits. `recoverNonfinalizedTransactions` asks Rust storage for hash/RLP recovery entries and stale-finalized classification; Rust removes stale pending rows before C++ validates hash/RLP consistency and inserts survivor payloads into Rust sidecars. `updateFinalizedTransactionsStatus` sends finalized hashes/RLP payloads to Rust; Rust plans count increments, stale recently-finalized period eviction, periodic queue purge, recently-finalized sidecar insertion, and non-finalized sidecar removal while persisting `TrxCount` before C++ mutates known-cache and pool side effects. C++ still owns locks, transaction materialization, pool/known-cache side effects, gas estimation/cache, FinalChain account fact reads, and broader lifecycle/finalization orchestration. |
