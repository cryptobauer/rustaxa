...
impl BridgeApp {
    ...
    /// Returns canonical current pillar-block bytes from the native owner.
    pub fn pbft_service_pillar_current_block_rlp(&self) -> Result<Vec<u8>> {
        self.0.current_pillar_block_rlp()
    }
}
