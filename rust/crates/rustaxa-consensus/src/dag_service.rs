...
use crate::dag::{
    ...
    DagPivotTipsValidation, DagProposerAttemptPlan, DagProposerAttemptPreparationInput,
    DagProposerAttemptVrfCompletionInput, DagProposerBlockIntentInput, DagProposerFrontierFacts,
    DagProposerPostPackInput, DagProposerRetryResetInput, DagProposerSignedBlockIntent,
    DagProposerSignedBlockIntentInput, DagProposerStaleProofInput,
    DagProposerStorageBlockConstructionInput, DagProposerStorageTipSelectionInput,
    DagProposerTipSelection, DagProposerUnsignedBlockIntent, DagProposerVdfWaitInput,
    ...
    complete_dag_proposer_attempt_vrf, construct_dag_vdf_message, dag_block_exists_in_storage,
    ...
    period_block_hash_from_storage, plan_dag_add_block_effects,
    ...
    plan_dag_verify_transaction_query, prepare_dag_proposer_attempt,
...
/// Key-custody VRF result for one exact proposer cursor.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DagProposerVrfReport {
    /// Legacy 80-byte VRF proof generated by the host-held wallet key.
    pub proof: Vec<u8>,
    /// Legacy 64-byte VRF output returned by the host executor.
    pub output: Vec<u8>,
}
...
impl DagServiceState {
    ...
    /// Applies period-selected gas policy to an active proposer session.
    ///
    /// The policy is selected only after FinalChain resolves the proposal
    /// period. Updating both retained construction limits and the attempt's
    /// transaction weight keeps packing and block construction on one schedule.
    pub(crate) fn configure_proposer_gas_policy(
        &mut self,
        session_id: u64,
        ...
        let session = self
            .proposer_sessions
            .get_mut(&session_id)
            .context("DAG_PROPOSER_GAS_POLICY_SESSION_NOT_ACTIVE")?;
        ensure!(
            matches!(session.action, DagProposerSessionAction::PackTransactions),
            "DAG_PROPOSER_GAS_POLICY_WRONG_STAGE"
        );
        session.attempt.proposal_weight_limit = proposal_weight_limit;
        session.begin_input.proposal_weight_limit = proposal_weight_limit;
        session.begin_input.pbft_gas_limit = pbft_gas_limit;
        session.begin_input.dag_gas_limit = dag_gas_limit;
        Ok(())
    }
    ...
    pub(crate) fn apply_proposer_final_chain_facts(
        ...
        let attempt_input = domain_attempt_preparation_input(
            &session.begin_input,
        ...
        let minimum_vdf_difficulty = sortition_params.vdf.difficulty_min;
        let attempt = match prepare_dag_proposer_attempt(attempt_input) {
            Ok(attempt) => attempt,
            Err(error) => {
                self.proposer_sessions.remove(&snapshot.session_id);
                return Err(error);
        ...
        let action = if attempt.action == DAG_PROPOSER_ACTION_CONTINUE {
            DagProposerSessionAction::ProveVrf
        } else {
            DagProposerSessionAction::Complete
        };
        let session = self
    ...
    /// Verifies one host-key-custody VRF report and completes proposer gating.
    ///
    /// The proof is checked against the cursor's public key and exact VRF
    /// message. Invalid shape/proofs remove only this cursor and return an
    /// error; stale/wrong-stage reports preserve the cursor as an invalid-report
    /// step. No secret key enters native configuration or state.
    pub(crate) fn report_proposer_vrf(
        &mut self,
        session_id: u64,
        ...
        if !matches!(session.action, DagProposerSessionAction::ProveVrf) {
            let session = self
                .proposer_sessions
                .get_mut(&session_id)
                .expect("session remains live");
            return Ok(invalid_dag_proposer_report(
                session,
                "DAG_PROPOSER_SESSION_UNEXPECTED_VRF_REPORT",
            ));
        }
        let proof: [u8; 80] = match report.proof.as_slice().try_into() {
            Ok(proof) => proof,
            Err(_) => {
                self.proposer_sessions.remove(&session_id);
                return Err(anyhow!("DAG_PROPOSER_VRF_PROOF_LENGTH_INVALID"));
            }
        };
        let reported_output: [u8; 64] = match report.output.as_slice().try_into() {
            Ok(output) => output,
            Err(_) => {
                self.proposer_sessions.remove(&session_id);
                return Err(anyhow!("DAG_PROPOSER_VRF_OUTPUT_LENGTH_INVALID"));
            }
        };
        let verified_output = vrf::verify_output(
            &session.begin_input.wallet_vrf_public_key,
            &proof,
            &session.attempt.vrf_input,
        )?
        .ok_or_else(|| anyhow!("DAG_PROPOSER_VRF_PROOF_INVALID"))?;
        ensure!(
            verified_output == reported_output,
            "DAG_PROPOSER_VRF_OUTPUT_MISMATCH"
        );
        let attempt = complete_dag_proposer_attempt_vrf(DagProposerAttemptVrfCompletionInput {
            prepared: session.attempt.clone(),
            frontier: session.observation.frontier.clone(),
            vrf_output: verified_output,
            sortition_params: session.sortition_params,
            max_non_finalized_dag_blocks: session.begin_input.max_non_finalized_dag_blocks,
            max_non_finalized_dag_blocks_low_difficulty: session
                .begin_input
                .max_non_finalized_dag_blocks_low_difficulty,
            last_propose_level: session.attempt.next_last_propose_level,
            retry_count: session.attempt.next_retry_count,
            max_retry_count: session.begin_input.max_retry_count,
        })?;
        let session = self
            .proposer_sessions
            .get_mut(&session_id)
            .expect("session remains live");
        session.status = if attempt.action == DAG_PROPOSER_ACTION_CONTINUE {
            DAG_PROPOSER_SESSION_STATUS_ACTIVE
        } else {
            DAG_PROPOSER_SESSION_STATUS_COMPLETE
        };
        session.action = if attempt.action == DAG_PROPOSER_ACTION_CONTINUE {
...
fn domain_attempt_preparation_input(
    input: &DagProposerSessionBeginInput,
    transaction_observation: DagProposerTransactionObservation,
    ...
    retry_count: u64,
) -> DagProposerAttemptPreparationInput {
    DagProposerAttemptPreparationInput {
...
mod tests {
    ...
    fn apply_proposer_final_chain_facts(
        ...
        let step = runtime.apply_proposer_final_chain_facts(
            &snapshot,
            0,
        ...
        let proof = vrf::prove(&TEST_VRF_SECRET, &step.vrf_input)?;
        let output =
            vrf::verify_output(&vrf_key, &proof, &step.vrf_input)?.context("test VRF output")?;
        runtime.report_proposer_vrf(
            session_id,
    ...
    fn begin_proposer_vdf_session(
        ...
        runtime.configure_proposer_gas_policy(session_id, 600, 6_000, 600)?;
        assert_eq!(
            runtime.proposer_pack_parameters(session_id)?.weight_limit,
            600
        );
        assert_eq!(
