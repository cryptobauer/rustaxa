...
| --- | --- | --- | --- | --- |
| `rust/crates/rustaxa-bridge/src/ffi.rs` | Plain CXX carriers plus declarations for all exported Rust bridge handles and functions | Generated CXX bridge, every C++ shim/external adapter using Rust | External boundary | Keep as the mechanically checked CXX boundary; delete declarations with their last caller and keep domain logic out of this module. |
...
| `rust/crates/rustaxa-bridge/src/dag.rs` | Private DAG runtime state plus direct CXX DAG command/result adapters | `dag_transaction_service.rs`, `dag_manager_shim`, `dag_block_proposer_shim`, FFI carrier conversion, Rust tests | Internal Rust route plus C++ executor adapter | Keep DAG state private behind `BridgeDagTransactionService`; retain direct command/result adapters while C++ executes VDF, signing, network, and public materialization effects, then delete each export with its final shim caller. |
...
| `rust/crates/rustaxa-bridge/src/pbft_period_cleanup.rs` | PBFT-service period-cleanup conversion and atomic cleanup entry points | `pbft_manager_shim`, service-owned vote/proposal state, Rust tests | Internal Rust route | Keep cleanup composed behind `BridgePbftService`; delete compatibility-only entry points after the remaining public vote/proposal facades retire. |
...
| `rust/crates/rustaxa-bridge/src/pbft_vote_generation.rs` | Signed-vote generation conversion and compatibility entry points | `vote_manager_shim`, payload tests | Internal Rust route | Keep only signing/materialization boundary conversion still called by C++; delete bridge helpers as vote production moves behind a typed signing port. |
| `rust/crates/rustaxa-bridge/src/pbft_vote_ingress.rs` | Network vote-ingress conversion and planner result mapping | `network.rs`, network API tests | Internal Rust route | Keep behind `BridgeConsensusNetworkApi` while tarcap owns transport; delete direct bridge helpers when ingress is fully internal to the network adapter. |
| `rust/crates/rustaxa-bridge/src/pbft_vote_payload.rs` | Canonical vote payload construction and inspection compatibility helpers | `vote_manager_shim`, PBFT/vote tests | C++ compatibility adapter | Retain only helpers required to materialize or inspect C++ `PbftVote`; delete them with that compatibility object boundary. |
| `rust/crates/rustaxa-bridge/src/pbft_vote_progress.rs` | Progress-context and executor-plan conversion helpers | `verified_votes.rs`, PBFT/vote tests | Internal Rust route | Keep conversions private to the composed PBFT service and delete bridge-only DTO mappings with their final CXX executor consumer. |
| `rust/crates/rustaxa-bridge/src/pbft_vote_validation.rs` | Canonical inspection and threshold-plan conversion helpers | `verified_votes.rs`, `vote_manager_shim`, PBFT/vote tests | Internal Rust route | Keep only the CXX inspection/materialization edge; validation facts and decisions stay private to `rustaxa-consensus`. |
...
- The first `CRW-10` checkpoint extends `rewrite_bridge_inventory_guard.sh` from exported-handle coverage to exact Rust
  bridge-module and consensus-shim-directory coverage. Missing and stale rows now fail the guard for all three inventory
