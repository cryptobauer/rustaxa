...
| PBFT manager | `pbft/pbft_manager.hpp`, `pbft/pbft_manager.cpp`, `pbft_manager_shim/*` | 3267 lines | `partial` | Split Rust services behind C++ daemon shell | Highest complexity: state machine, finalization, gossip, threading, storage, DAG, votes, pillar, FinalChain. Rust-enabled pillar-vote sync validation now uses a temporary header overlay plus shim-owned helper that calls the Rust pillar-vote bundle planner for deterministic period/block/prevalidation/unique-weight threshold decisions and then applies the existing live C++ side effects in shim-owned code. |
