...
Recommended next selection: continue `CRW-03` with atomic vote/proposed cleanup before selecting the broader
sibling-service work in `CRW-04` or `CRW-05`. Leader snapshot/revalidation and admission-plus-progress persistence are
complete.
...
The item remains active for one combined-operation debt exposed by the ownership work: period advance cleans vote and
proposed-block state through separate actions. Authoritative leader selection now uses one snapshot/revalidation
contract, and vote admission now commits required progress rows before publishing the live transition. Period cleanup
must become one atomic service operation before CRW-03 can close, alongside the remaining PBFT-private
rewards/sortition deletion conditions.
