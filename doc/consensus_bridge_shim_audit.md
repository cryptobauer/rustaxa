...
| `rust/crates/rustaxa-bridge/src/pillar_votes.rs` | `BridgePillarVotes`, `create_pillar_votes_index` | `pillar_chain_manager_shim` (direct in-memory aggregation), period-data/vote paths | Internal Rust route | The C++ `pillar_votes_shim` facade is retired. Keep this bridge handle only while `pillar_chain_manager_shim` owns vote indexing/inspection in Rust and still materializes verified votes through payload lookup. |
...
| `BridgePillarVotes` | `pillar_votes.rs` | `pillar_chain_manager_shim` and period-data/vote paths | Internal Rust route | Pillar vote indexing/admission is Rust-internal state for the current shim; remove when a native pillar-chain runtime API bypasses this bridge handle. |
