...
cache and transaction-manager side effects. Rust-mode non-finalized DAG sync reads the
...
| Define consensus storage ports | `partial` | Started narrowly with `BridgeDagManagerRuntime` storage-backed DAG sync and finalization cleanup payload assembly: Rust selects non-finalized hashes, loads selected DAG block RLPs through Rust storage, decodes transaction references, performs shared Rust transaction RLP lookup, resolves finalized transaction status, returns finalized-block counter facts, and routes expired DAG block deletes through Rust storage. Broader typed storage traits for consensus services are still needed before PBFT/DAG finalization side effects move fully into Rust. |
