...
| `rust/crates/rustaxa-bridge/src/dag.rs` | `BridgeDagManagerRuntime` | `dag_manager_shim`, DAG manager/proposer tests | C++ public compatibility facade | Native `DagManagerState` owns both DAG graphs. Keep narrowing the manager runtime until the C++ manager facade becomes a thin external executor API. |
