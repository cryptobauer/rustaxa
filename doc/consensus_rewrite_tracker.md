...
| `TransactionManager` | verify/insert/pack/get/finalize status/non-finalized recovery/gas estimation | `DbStorage`, `FinalChain`, thread pool, `DagBlock`, state API | `transaction_test`, `transaction_manager_shim_test`, `dag_block_test`, `pbft_manager_test`, `full_node_test` | Rust-backed packing, DAG persistence, finalized-status planning, storage-backed transaction lookup, non-finalized recovery reads, and finalized filter/verification helpers with C++ live state/materialization/estimation shell; live pool/non-finalized/count read helpers are shim-owned; continue moving verification/insertion and lifecycle mutation before broader orchestration |
