...
Recommended next selection: continue `CRW-03` with one bounded combined-operation debt—leader snapshot/revalidation,
admission-plus-progress persistence, or atomic vote/proposed cleanup—before selecting the broader sibling-service work
in `CRW-04` or `CRW-05`.
...
The item remains active for three combined-operation debts exposed by the ownership work: authoritative leader
selection still reads proposal votes and proposed blocks in separate service calls, admission persists some progress
effects after the live runtime transition, and period advance cleans vote/proposed state through separate actions.
These must become narrow snapshot/revalidate or atomic service operations before CRW-03 can close, alongside the
remaining PBFT-private rewards/sortition deletion conditions.
...
| Verified votes | `vote_manager/verified_votes.hpp`, `vote_manager/verified_votes.cpp`, `verified_votes_shim/*` | 384 lines | `rust-backed` | PBFT application-service state behind C++ materialization facade | Unique-voter checks, voted-value weight aggregation, 2t+1 block mappings, period cleanup, and round t+1 markers route to `rustaxa-consensus::verified_votes` through `RUSTAXA_ENABLE_VERIFIED_VOTES`. Production `BridgePbftService` restores and owns `PbftVoteAdmissionRuntime` behind its vote mutex; the chain-only service has no vote runtime, and storage-free construction is private Rust test scaffolding. `BridgeVerifiedVotes`, its factory, the facade-owned box, and the C++ mutex are deleted. C++ reconstructs transient `PbftVote` objects only from owned compatibility results; full state, step buckets, current rewards, and mutation outcomes are captured under one vote-lock epoch. Get-next-votes egress uses a Rust plan/build API returning ordered vote hashes and optimized bundle RLP bytes. The standalone C++ facade owns exact compatibility carriers but no authoritative state; Rust-enabled builds do not import or compile `VerifiedVotesOld`. Remaining CRW-03 debt is combined leader, admission-persistence, and period cleanup, not verified-vote ownership. |
