...
| Pillar vote aggregation | Rust validation plus `pillar_votes_shim_test`; broaden to `pillar_chain_test` when manager behavior is touched |
...
| Route PillarVotes aggregation through Rust/shim | `rust-backed` | Landed under `RUSTAXA_ENABLE_PILLAR_VOTES`; Rust owns period initialization, uniqueness, weighted block aggregation, threshold subset selection, and cleanup. C++ owns live `PillarVote` sidecars keyed by Rust-selected hashes and throws if a Rust-selected vote is missing locally. Direct `PillarVotesOld` vs Rust shim parity coverage now exercises period initialization, duplicate idempotence, same-voter conflict rejection, distinct-weight threshold selection, and cleanup. Signing/recovery and `PillarChainManager` orchestration remain future work. |
