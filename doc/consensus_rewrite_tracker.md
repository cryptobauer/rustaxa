...
| Route PillarVotes aggregation through Rust/shim | `rust-backed` | Landed under `RUSTAXA_ENABLE_PILLAR_VOTES`; Rust owns period initialization, uniqueness, weighted block aggregation, threshold subset selection, and cleanup. C++ owns live `PillarVote` sidecars keyed by Rust-selected hashes and throws if a Rust-selected vote is missing locally. Signing/recovery and `PillarChainManager` orchestration remain future work. |
