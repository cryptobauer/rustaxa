...
impl FinalChainRewardSchedulerRuntime {
    ...
    ///
    /// Timer zero matches a newly constructed Go Contract. The first StateAPI
    /// epoch must still be supplied by the serialized owner and can be consumed
    /// only once; absence of a pending successor never proves startup.
    pub(crate) fn new(published_head: FinalChainBlockNumber) -> Self {
        Self {
            ...
            epoch: FinalChainRewardSchedulerEpochState::FreshStartup,
            admitted: None,
            pending: None,
            last_installed: None,
        }
    ...
    /// Consumes the one fresh-startup epoch without advancing the durable head.
    pub(crate) fn bind_fresh_startup(&mut self, state_api_epoch: u64) -> anyhow::Result<()> {
        anyhow::ensure!(
            state_api_epoch != 0,
            "FINAL_CHAIN_REWARD_SCHEDULER_EPOCH_ZERO"
        );
        anyhow::ensure!(
            self.epoch == FinalChainRewardSchedulerEpochState::FreshStartup,
            "FINAL_CHAIN_REWARD_SCHEDULER_FRESH_STARTUP_ALREADY_CONSUMED"
        );
        self.epoch = FinalChainRewardSchedulerEpochState::Live(state_api_epoch);
        Ok(())
    }

    /// Consumes a fresh owner epoch once or validates the already-live owner.
    /// The caller must have obtained the epoch from a serialized StateAPI
    /// preflight belonging to this FinalChain application instance.
    pub(crate) fn observe_live_epoch(&mut self, state_api_epoch: u64) -> anyhow::Result<()> {
        match self.epoch {
            FinalChainRewardSchedulerEpochState::FreshStartup => {
                self.bind_fresh_startup(state_api_epoch)
            }
    ...
    /// Consumes fresh startup while recovery advances FinalChain by one period.
    ///
    /// This is valid only when the application proves that this FinalChain and
    /// the reported StateAPI epoch were newly constructed together. It cannot
    /// be selected from an absent pending candidate.
    pub(crate) fn install_fresh_startup_recovery(
        &mut self,
        state_api_epoch: u64,
        expected_parent: FinalChainBlockNumber,
        ...
        anyhow::ensure!(
            self.published_head == expected_parent
                && expected_parent.checked_next() == Some(committed_head),
            "FINAL_CHAIN_REWARD_SCHEDULER_FRESH_RECOVERY_HEAD_MISMATCH"
        );
        self.bind_fresh_startup(state_api_epoch)?;
    ...
    /// Consumes fresh startup after recovery proves the durable publication
    /// was already present when this FinalChain instance was constructed.
    pub(crate) fn install_fresh_startup_at_durable_head(
        &mut self,
        state_api_epoch: u64,
        committed_head: FinalChainBlockNumber,
    ) -> anyhow::Result<()> {
    ...
    pub(crate) fn install_recovered_publication(
        ...
        if self.epoch == FinalChainRewardSchedulerEpochState::FreshStartup {
            if self.published_head == binding.period {
                self.install_fresh_startup_at_durable_head(state_api_epoch, binding.period)?;
            } else {
                self.install_fresh_startup_recovery(
                    state_api_epoch,
                    binding.expected_parent,
                    binding.period,
                )?;
            }
    ...
    /// Applies a verified StateAPI discard/re-init boundary.
    ///
    /// Reopen resets the Go timer even though the committed head is unchanged.
    /// Every outstanding candidate from the old epoch becomes invalid.
    pub(crate) fn reset_after_verified_reopen(
        &mut self,
...
mod tests {
    ...
    #[test]
    fn fresh_startup_is_consumed_once_per_runtime_instance() {
        let mut runtime = FinalChainRewardSchedulerRuntime::new(7.into());
        runtime.bind_fresh_startup(11).unwrap();
        assert!(runtime.bind_fresh_startup(11).is_err());
        assert!(runtime.bind_fresh_startup(12).is_err());
    }
    ...
    fn foreign_final_chain_instance_cannot_arm_coincident_generation() {
        ...
        first.bind_fresh_startup(11).unwrap();
        second.bind_fresh_startup(11).unwrap();
    ...
    fn verified_reopen_replaces_epoch_resets_timer_and_invalidates_pending() {
        ...
        runtime
            .reset_after_verified_reopen(11, 12, 7.into())
            .unwrap();
    ...
    #[test]
    fn fresh_recovery_advances_with_zero_timer_only_once() {
        let mut runtime = FinalChainRewardSchedulerRuntime::new(7.into());
        runtime
            .install_fresh_startup_recovery(11, 7.into(), 8.into())
            .unwrap();
        assert_eq!(runtime.next_cleanup_block, 0);
        assert!(
            runtime
                .install_fresh_startup_recovery(11, 8.into(), 9.into())
                .is_err()
        );
        assert!(
            runtime
    ...
    fn verified_reopen_resets_cached_skip_without_persisting_timer() {
        ...
        runtime
            .reset_after_verified_reopen(11, 12, 3.into())
            .unwrap();
