...
| Pillar block/votes | `pillar_chain/pillar_block.hpp`, `pillar_chain/pillar_votes.hpp`, matching `.cpp` files | 627 lines | `rust-domain` for pillar types/codecs and vote aggregation | Rust domain and codecs | `rustaxa-types::pillar` now mirrors `PillarBlock`, `ValidatorVoteCountChange`, `PillarVote`, `PillarBlockData`, optimized pillar-vote bundle, and current pillar data RLP/Solidity/hash shapes with malformed-input coverage and C++ golden-vector coverage for Solidity bytes. `rustaxa-consensus::pillar_votes` owns already-verified vote uniqueness, weighted per-block aggregation, deterministic threshold subset selection, and cleanup. Vote signing/recovery, manager orchestration, JSON/RPC, and production routing are not started. |
...
| `PillarVotes` | vote uniqueness, threshold accumulation, above-threshold selection, cleanup | `PillarVote` | Rust `pillar_votes` unit tests; future `pillar_chain_test` shim parity | Rust domain implemented for already-verified vote facts; C++ production routing deferred |
