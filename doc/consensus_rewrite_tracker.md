...
Rust `DagManagerState` directly, including empty-period advancement, and Rust-mode non-finalized DAG sync reads the
...
planning and expired-block transaction cleanup selection also route through Rust hash plans while C++ still owns cache
cleanup, DB side-effect calls, and other transaction-manager side effects.
...
| Define consensus storage ports | `partial` | Started narrowly with `BridgeDagManagerRuntime` storage-backed DAG sync payload assembly: Rust selects non-finalized hashes, loads selected DAG block RLPs through Rust storage, decodes transaction references, and performs shared Rust transaction RLP lookup. Broader typed storage traits for consensus services are still needed before PBFT/DAG finalization side effects move fully into Rust. |
