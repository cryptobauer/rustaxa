...
class PbftManager {
 public:
  class EligibleWallets {
   public:
    ...
    /**
     * Returns the configured signing identity at the stable application wallet index.
     *
     * Native consensus uses the same application-order index when emitting a slashing transaction effect. This
     * accessor exposes only the retained signing leaf and does not consult or mutate DPoS eligibility. Invalid indices
     * throw `std::out_of_range`; returned references remain valid for the lifetime of this wallet collection.
     */
    const WalletConfig &getSigningWallet(size_t wallet_index) const;
 ...
 public:
   ...
   *
   * The shim sends compact validation facts plus canonical `PeriodData` RLP bytes
   * to Rust so Rust can own queue ordering and admission state without holding
   * live C++ payload objects.
   */
 ...
 private:
   ...
   * executes only the temporary compatibility effects still owned by the shim: timers, counters, and wallet
   * eligibility. Rust remains the source of ordering, owns combined vote/proposed-block cleanup, and publishes the
   * runtime snapshot only after durable cleanup succeeds. Every completed external action is reported back to Rust
   * before that native commit.
   *
   * Returns false when Rust rejects the plan, cleanup commit, or resulting runtime period snapshot.
   */
   ...
   *
   * Rust owns queue ordering, period admission, cleanup, and pop-source decisions
   * inside `pbft_manager_runtime_`. Current-certificate votes remain canonical
   * Rust bridge bytes until the native PBFT service accepts and weights them;
   * only accepted output is materialized for the retained C++ push boundary.
   */
  struct PoppedPeriodDataPayload {
    PeriodData period_data;
    rust::Vec<rustaxa::PbftCertVoteRlp> cert_vote_rlps;
  ...
  // No C++ queue-sidecar state; Rust owns queue metadata and admission state.
