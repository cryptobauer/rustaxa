...
| `rust/crates/rustaxa-bridge/src/pillar_votes.rs` | Standalone pillar-vote inspection APIs plus Rust-internal `BridgePillarVotes` compatibility helpers and runtime-owned pillar-chain methods | Live `pillar_chain_manager_shim` uses `BridgePillarChainRuntime`; C++ bridge tests use the runtime or standalone inspection APIs | Internal Rust route | The C++ `pillar_votes_shim` facade and standalone `BridgePillarVotes` CXX handle are retired. Keep the Rust-only helper only while bridge-module tests need direct fixture state; production C++ callers must use `BridgePillarChainRuntime` or native Rust modules. |
