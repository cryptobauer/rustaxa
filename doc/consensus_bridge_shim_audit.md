...
| `rust/crates/rustaxa-bridge/src/dag_transaction_service.rs` | `BridgeDagTransactionService` | App bootstrap, `dag_manager_shim`, `transaction_manager_shim`, `gas_pricer_shim`, DAG/transaction compatibility tests | Application/bootstrap handle | One App-owned service contains private DAG and transaction state behind sibling mutexes and a shared Rust storage owner. Full construction restores both domains and the initial proposal mapping before publication; compatibility construction omits DAG state and returns `DAG_SERVICE_UNAVAILABLE` for DAG calls. Keep until retained public facades and external EVM/VDF/signing/network executors move behind narrower application APIs. |
