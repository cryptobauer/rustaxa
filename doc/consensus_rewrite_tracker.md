...
returns a single Rust snapshot with period and selected hashes, non-finalized transaction query planning routes through
Rust hash plans, and sync transaction body lookup reads Rust storage RLP bytes directly before the C++ shim constructs
legacy `Transaction` objects. Expired-block transaction cleanup selection also routes through Rust hash plans while C++
still owns cache cleanup, DB side-effect calls, and other transaction-manager side effects.
