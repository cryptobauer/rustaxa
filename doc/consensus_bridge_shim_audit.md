...
| `rust/crates/rustaxa-bridge/src/pbft_manager.rs` | `BridgePbftService`, `create_pbft_service_from_storage`, bootstrap/runtime/sync/finalization methods | App bootstrap, `pbft_manager_shim`, `pbft_chain_shim`, `vote_manager_shim`, `slashing_manager_shim`, PBFT bridge tests | Application/bootstrap handle and internal Rust route | One service owns manager, chain, proposed-block, verified-vote, and slashing state. CRW-03 closed the leader, admission/persistence, and period-cleanup crossings; CRW-05 absorbed the standalone slashing planner and duplicate cache. Continue narrowing CXX operations until the C++ manager facade contains only classified lifecycle/executor/materialization work. |
...
| `rust/crates/rustaxa-bridge/src/sortition.rs` | Sortition methods on `BridgeDagTransactionService` | `sortition_params_manager_shim`, DAG/proposer and PBFT-finalization paths | Application-service state plus C++ compatibility facade | The standalone handle and factories are deleted. Keep the service methods while DAG/proposer and PBFT finalization cross the C++ facade; internalize their remaining fact relays before retiring that facade. |
...
| `BridgePbftService` | `pbft_manager.rs`, `pbft_chain.rs`, `pbft_sync.rs`, `proposed_blocks.rs`, `verified_votes.rs`, `slashing.rs` | App bootstrap, PBFT/vote/proposed-block/slashing shims, PBFT/storage bridge tests | Application/bootstrap handle and internal Rust route | Owns manager, chain, proposed-block, verified-vote, and slashing state behind sibling locks. Keep the chain-only constructor only for the stable public C++ compatibility adapter; narrow manager operations as external executors gain typed ports. |
...
| --- | --- | --- | --- | --- |
| `dag_block_proposer_shim` | Standalone C++ executor facade over Rust manager-owned proposal sessions; C++ executes VDF, signs one returned hash, and submits the returned signed RLP; feature-on builds do not compile the original proposer or an Old scaffold | App/DAG manager proposer lifecycle | C++ public compatibility facade | Delete after worker/network lifecycle, VDF execution, signing, and add-block effects move behind Rust application/external ports. |
...
| `sortition_params_manager_shim` | Service-backed sortition compatibility facade with shim-owned compatibility carrier/codec and no independent Rust handle | DAG/sortition and PBFT-finalization paths | C++ public compatibility facade | `SortitionParamsManagerOld`, the standalone Rust handle/factories, and the redundant module flag are retired; master Rust mode injects the canonical DAG/transaction service. Delete after DAG/proposer and PBFT finalization use native service operations without the facade. |
