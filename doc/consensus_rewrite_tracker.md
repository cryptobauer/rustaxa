...
upstream manager. The checked surface is now 14,340 bridge lines, 7,628 shim lines, 277 CXX functions, 217 carriers,
14 handles, six shim directories, and 33 non-test bridge consumers. Six per-domain storage-query wrappers also collapse
to one read-only compatibility handle so the exact host ports still produce a net handle reduction.
...
| DAG, transaction, proposer | DAG/transaction family private under `CRW-12` root | Affected `CRW-14` facades and `CRW-15` object families | DAG portion of `CRW-16`; retain only named VDF/signing/network/EVM leaves |
...
`FullNodeTest.sync_five_nodes` covers sync continuation, transaction convergence, and pruning. The inventory now measures
14,340 bridge lines, 7,628 shim lines, 277 CXX functions, 217
carriers, 14 handles, six shim directories, and 33 non-test C++ consumers.
