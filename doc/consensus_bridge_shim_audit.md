...
| `rust/crates/rustaxa-bridge/src/sortition.rs` | Sortition methods on `BridgeDagTransactionService` | `sortition_params_manager_shim` and the PBFT cross-service finalization operation | Application-service state plus C++ compatibility facade | The standalone handle and factories are deleted. PBFT finalization atomically publishes sortition state through the two application-owned services, while DAG verification and proposal load historical parameters inside the composed DAG service; all three C++ report/fact bounces are deleted. The facade remains for public compatibility plus the PBFT accessor/preview path pending a separate retirement audit. |
