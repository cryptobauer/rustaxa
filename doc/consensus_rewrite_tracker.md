...
| PBFT manager | `pbft/pbft_manager.hpp`, `pbft/pbft_manager.cpp`, `pbft_manager_shim/*` | 3267 lines | `partial` | Split Rust services behind C++ daemon shell | Highest complexity: state machine, finalization, gossip, threading, storage, DAG, votes, pillar, FinalChain. Rust-enabled pillar-vote sync validation now uses a shim-owned helper that calls the Rust pillar-vote bundle planner for deterministic period/block/prevalidation/unique-weight threshold decisions before C++ applies live vote side effects. |
...
| Pillar vote aggregation and sync bundle validation | Rust validation plus `pillar_votes_shim_test`; broaden to `pbft_manager_test` and `pillar_chain_test` when manager behavior is touched |
...
| Route PBFT manager pillar-vote bundle validation through Rust | `partial` | The Rust-enabled sync path calls a shim-owned Rust bundle planner after C++ gathers signature and DPoS vote-count facts, but before C++ mutates `PillarChainManager`. This tightens deterministic threshold accounting to unique accepted votes and prevents duplicate bundle entries from satisfying threshold by recount. Remaining PBFT manager state machine, proposal handling, finalization, and gossip orchestration are still C++ owned. |
| Restore `pbft_manager.cpp` upstream shape after complete shim | `partial` | This slice adds a guarded call from the upstream-owned PBFT manager file into `pbft_manager_shim` because no complete `PbftManager` overlay exists yet. Treat that touch point as temporary merge-conflict debt: once a full `PbftManager` shim owns Rust-mode routing, move the hook into the shim and revert `libraries/core_libs/consensus/src/pbft/pbft_manager.cpp` back to upstream shape. |
