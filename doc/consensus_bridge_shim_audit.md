...
| `rust/crates/rustaxa-bridge/src/transaction_manager.rs` | `BridgeTransactionManagerRuntime`, admission execution/session helpers | `transaction_manager_shim`, RPC submission paths, tests | C++ public compatibility facade | The standalone sidecar handle is retired; live sidecar state is private runtime state. Delete the remaining runtime bridge after the transaction manager public C++ facade is retired or all admission/packing paths are native Rust. Keep external EVM/final-chain callbacks as a minimal API. |
