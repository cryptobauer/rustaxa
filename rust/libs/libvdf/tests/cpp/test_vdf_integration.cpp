...
TEST_F(VDFIntegrationTest, MainExampleTest) {
  const auto vdf1 = make_vdf(20, 8, {97}, {213, 166, 245, 127, 146, 139, 45, 0});
  ...
  const auto vdf3 = make_vdf(20, 8, {77, 39, 11}, {213, 166, 245, 127, 146, 139, 45, 0});
...
TEST_F(VDFIntegrationTest, ConsistencyTest) {
  rust::Vec<uint8_t> input = {42, 123, 255};
  rust::Vec<uint8_t> modulus = {213, 166, 245, 127, 146, 139, 45, 0};
  auto cancellation_token = make_cancellation_token();

  // Create multiple VDFs with same parameters
  auto vdf1 = make_vdf(20, 7, input, modulus);  // Reduced time_bits for performance
  auto vdf2 = make_vdf(20, 7, input, modulus);
...
TEST_F(VDFIntegrationTest, EdgeCaseParameters) {
  ...
  // assert the result
  auto vdf_empty = make_vdf(20, 6, {}, {213, 166, 245, 127, 146, 139, 45, 0});
...
TEST_F(VDFIntegrationTest, DifferentModulusTest) {
  rust::Vec<uint8_t> input = {97};
  auto cancellation_token = make_cancellation_token();

  auto vdf1 = make_vdf(20, 6, input, {7, 11});
  auto vdf2 = make_vdf(20, 6, input, {13, 17});
  auto vdf3 = make_vdf(20, 6, input, {213, 166, 245, 127, 146, 139, 45, 0});
...
TEST_F(VDFIntegrationTest, PerformanceCharacteristics) {
  auto vdf_fast = make_vdf(20, 4, {97}, {213, 166, 245, 127, 146, 139, 45, 0});  // 2^4 = 16 iterations
  auto vdf_slow = make_vdf(20, 6, {97}, {213, 166, 245, 127, 146, 139, 45, 0});  // 2^6 = 64 iterations
  auto cancellation_token = make_cancellation_token();
