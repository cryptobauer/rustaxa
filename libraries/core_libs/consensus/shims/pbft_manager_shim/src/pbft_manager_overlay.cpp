...
constexpr uint8_t kPbftSyncStatusRewardVotesInvalid = 6;
...
constexpr uint8_t kPbftManagerLeaderCandidateReady = 0;
constexpr uint8_t kPbftManagerLeaderCandidateNullVoteBlockHash = 1;
constexpr uint8_t kPbftManagerLeaderCandidateBlockInChain = 2;
constexpr uint8_t kPbftManagerLeaderCandidateBlockMissingOrInvalid = 3;
constexpr uint8_t kPbftManagerLeaderCandidateInvalidVoteWeight = 4;
...
rustaxa::PbftFinalChainFactRequest makePbftFinalChainFactRequest(PbftPeriod period,
                                                                 ...
                                                                 bool collect_total_vote_count,
                                                                 bool collect_address_vote_counts,
...
rustaxa::PbftManagerTransitionFact makePbftManagerTransitionFact(
    ...
    std::chrono::milliseconds deadline, std::chrono::milliseconds next_step_time,
    const VoteManager &vote_mgr, bool cacti_hardfork, bool has_cert_voted_block, bool executed_pbft_block) {
  ...
  const auto next_step = kind == kPbftManagerTransitionResetConsensus
                             ? PbftStep{1}
                             : (kind == kPbftManagerTransitionDelayCertifyPoll ||
                                        kind == kPbftManagerTransitionDelayFinishPoll
                                    ? step
                                    : step + 1);
...
rustaxa::PbftManagerStateActionFact makePbftManagerStateActionFact(
    ...
    const std::optional<std::shared_ptr<PbftBlock>> &cert_voted_block_for_round, bool already_next_voted_value,
  ...
  const auto needs_current_round_soft =
      state == finish_polling_state ||
      (state == certify_state && certify_vote_window_started && !certify_will_finish);

  if (round >= 2 && needs_previous_round_next_null) {
    fact.has_previous_round_next_null =
        vote_mgr.getTwoTPlusOneVotedBlock(period, round - 1, TwoTPlusOneVotedBlockType::NextVotedNullBlock)
            .has_value();
  }
  ...
  if (cert_voted_block_for_round.has_value()) {
    fact.has_cert_voted_block = true;
    fact.cert_voted_block_hash = toBridgeHash((*cert_voted_block_for_round)->getBlockHash());
  }
...
uint8_t toPbftSyncFinalChainStatus(PbftStateRootValidation status) {
  switch (status) {
    ...
    case PbftStateRootValidation::Missing:
      return kPbftSyncFinalChainMissing;
    case PbftStateRootValidation::Invalid:
  ...
  while (!stopped_) {
    ...
    auto apply_delay_transition = [&](uint8_t kind) {
      ...
      const auto plan = rustaxa::plan_pbft_manager_transition(makePbftManagerTransitionFact(
          kind, current_period, current_round, step_, 0, current_round_lambda_,
          ...
          next_step_time_ms_, *vote_mgr_, kGenesisConfig.state.hardforks.isOnCactiHardfork(current_period),
          cert_voted_block_for_round_.has_value(), executed_pbft_block_));
    ...
    while (!stopped_) {
      ...
      switch (step.action) {
        ...
        case kPbftManagerRuntimeActionDelayCertifyPoll: {
          ...
          step = report_action(step, applied ? kPbftManagerRuntimeResultSleepApplied
                                             : kPbftManagerRuntimeResultExecutorError,
...
void PbftManager::resetPbftConsensus(PbftRound round) {
  ...
  const auto plan = rustaxa::plan_pbft_manager_transition(makePbftManagerTransitionFact(
      kPbftManagerTransitionResetConsensus, period, getPbftRound(), step_, round, current_round_lambda_,
      std::chrono::milliseconds(getRoundLambda(round)), std::chrono::milliseconds(kGenesisConfig.pbft.lambda_ms),
      kMaxExponentialLambda, getPbftDeadline(), next_step_time_ms_, *vote_mgr_,
      kGenesisConfig.state.hardforks.isOnCactiHardfork(period), cert_voted_block_for_round_.has_value(),
...
void PbftManager::setFilterState_() {
  const auto [round, period] = getPbftRoundAndPeriod();
  const auto plan = rustaxa::plan_pbft_manager_transition(makePbftManagerTransitionFact(
      kPbftManagerTransitionToFilter, period, round, step_, 0, current_round_lambda_,
      std::chrono::milliseconds(getRoundLambda(round)), std::chrono::milliseconds(kGenesisConfig.pbft.lambda_ms),
      kMaxExponentialLambda, getPbftDeadline(), next_step_time_ms_, *vote_mgr_,
      kGenesisConfig.state.hardforks.isOnCactiHardfork(period), cert_voted_block_for_round_.has_value(),
      executed_pbft_block_));
...
void PbftManager::setCertifyState_() {
  const auto [round, period] = getPbftRoundAndPeriod();
  const auto plan = rustaxa::plan_pbft_manager_transition(makePbftManagerTransitionFact(
      kPbftManagerTransitionToCertify, period, round, step_, 0, current_round_lambda_,
      std::chrono::milliseconds(getRoundLambda(round)), std::chrono::milliseconds(kGenesisConfig.pbft.lambda_ms),
      kMaxExponentialLambda, getPbftDeadline(), next_step_time_ms_, *vote_mgr_,
      kGenesisConfig.state.hardforks.isOnCactiHardfork(period), cert_voted_block_for_round_.has_value(),
      executed_pbft_block_));
...
void PbftManager::setFinishState_() {
  ...
  const auto [round, period] = getPbftRoundAndPeriod();
  const auto plan = rustaxa::plan_pbft_manager_transition(makePbftManagerTransitionFact(
      kPbftManagerTransitionToFinish, period, round, step_, 0, current_round_lambda_,
      std::chrono::milliseconds(getRoundLambda(round)), std::chrono::milliseconds(kGenesisConfig.pbft.lambda_ms),
      kMaxExponentialLambda, getPbftDeadline(), next_step_time_ms_, *vote_mgr_,
      kGenesisConfig.state.hardforks.isOnCactiHardfork(period), cert_voted_block_for_round_.has_value(),
      executed_pbft_block_));
...
void PbftManager::setFinishPollingState_() {
  const auto [round, period] = getPbftRoundAndPeriod();
  const auto plan = rustaxa::plan_pbft_manager_transition(makePbftManagerTransitionFact(
      kPbftManagerTransitionToFinishPolling, period, round, step_, 0, current_round_lambda_,
      std::chrono::milliseconds(getRoundLambda(round)), std::chrono::milliseconds(kGenesisConfig.pbft.lambda_ms),
      kMaxExponentialLambda, getPbftDeadline(), next_step_time_ms_, *vote_mgr_,
      kGenesisConfig.state.hardforks.isOnCactiHardfork(period), cert_voted_block_for_round_.has_value(),
      executed_pbft_block_));
...
void PbftManager::loopBackFinishState_() {
  const auto [round, period] = getPbftRoundAndPeriod();
  const auto plan = rustaxa::plan_pbft_manager_transition(makePbftManagerTransitionFact(
      kPbftManagerTransitionLoopBackFinish, period, round, step_, 0, current_round_lambda_,
      std::chrono::milliseconds(getRoundLambda(round)), std::chrono::milliseconds(kGenesisConfig.pbft.lambda_ms),
      kMaxExponentialLambda, getPbftDeadline(), next_step_time_ms_, *vote_mgr_,
      kGenesisConfig.state.hardforks.isOnCactiHardfork(period), cert_voted_block_for_round_.has_value(),
      executed_pbft_block_));
...
void PbftManager::proposeBlock_() {
  ...
  const auto plan = rustaxa::plan_pbft_manager_state_action(makePbftManagerStateActionFact(
      state_, period, round, step_, 0ms, getPbftDeadline(), current_round_lambda_, *vote_mgr_,
      cert_voted_block_for_round_, already_next_voted_value_, already_next_voted_null_block_hash_));
  if (!ensureStateActionPlanReady(plan, log_er_)) {
    return;
  }

  if (plan.primary_intent == kPbftManagerStateActionIntentProposeNewBlock) {
  ...
  if (plan.primary_intent == kPbftManagerStateActionIntentReproposePreviousRoundNextValue) {
    assert(round > 1);
  ...
  if (plan.primary_intent != kPbftManagerStateActionIntentNoop) {
    LOG(log_er_) << "Unsupported Rust PBFT value proposal intent " << static_cast<uint32_t>(plan.primary_intent);
    assert(false);
  }
}
...
void PbftManager::identifyBlock_() {
  ...
  if (plan.primary_intent == kPbftManagerStateActionIntentIdentifyLeaderAndSoftVote) {
  ...
  if (plan.primary_intent != kPbftManagerStateActionIntentNoop) {
    LOG(log_er_) << "Unsupported Rust PBFT filter intent " << static_cast<uint32_t>(plan.primary_intent);
    assert(false);
  }
}
...
void PbftManager::certifyBlock_() {
  ...
  const auto plan = rustaxa::plan_pbft_manager_state_action(makePbftManagerStateActionFact(
      state_, period, round, step_, elapsed_time_in_round, getPbftDeadline(), current_round_lambda_, *vote_mgr_,
      cert_voted_block_for_round_, already_next_voted_value_, already_next_voted_null_block_hash_));
  if (!ensureStateActionPlanReady(plan, log_er_)) {
    return;
  }

  go_finish_state_ = plan.go_finish_state;
  if (plan.primary_intent == kPbftManagerStateActionIntentGoFinish) {
  ...
  if (plan.primary_intent != kPbftManagerStateActionIntentCertVoteCurrentSoftValue) {
    LOG(log_er_) << "Unsupported Rust PBFT certify intent " << static_cast<uint32_t>(plan.primary_intent);
    assert(false);
    return;
  }
...
void PbftManager::firstFinish_() {
  ...
  if (plan.primary_intent == kPbftManagerStateActionIntentNextVoteCertVotedBlock) {
  ...
  if (plan.primary_intent == kPbftManagerStateActionIntentNextVotePreviousRoundValue) {
  ...
  if (plan.primary_intent != kPbftManagerStateActionIntentNoop) {
    LOG(log_er_) << "Unsupported Rust PBFT first-finish intent " << static_cast<uint32_t>(plan.primary_intent);
    assert(false);
  }
}
...
void PbftManager::secondFinish_() {
  ...
  const auto plan = rustaxa::plan_pbft_manager_state_action(makePbftManagerStateActionFact(
      state_, period, round, step_, elapsedTimeInMs(second_finish_step_start_datetime_), getPbftDeadline(),
      current_round_lambda_, *vote_mgr_, cert_voted_block_for_round_, already_next_voted_value_,
      already_next_voted_null_block_hash_));
  if (!ensureStateActionPlanReady(plan, log_er_)) {
    return;
  }
  ...
  if (plan.primary_intent == kPbftManagerStateActionIntentNextVoteCurrentSoftValue) {
    ...
    LOG(log_er_) << "Unsupported Rust PBFT second-finish primary intent "
                 << static_cast<uint32_t>(plan.primary_intent);
    assert(false);
    return;
  }
  ...
  loop_back_finish_state_ = plan.loop_back_finish_state;
}
...
std::optional<PbftManager::ProposedBlockData> PbftManager::generatePbftBlock(
  ...
  const auto final_chain_facts = final_chain_->rustFinalChainForRust().collect_pbft_final_chain_facts(
      makePbftFinalChainFactRequest(propose_period, kNullBlockHash, true, false, false, false));
  if (final_chain_facts.final_chain_hash.status != kPbftSyncFinalChainValid) {
    LOG(log_wr_) << "Block for period " << propose_period << " could not be proposed as we are behind. Rust "
                 << "FinalChain hash status " << static_cast<uint32_t>(final_chain_facts.final_chain_hash.status)
                 << ", error " << static_cast<std::string>(final_chain_facts.final_chain_hash.error_code);
    return {};
  }
  auto final_chain_hash = fromBridgeHash(final_chain_facts.final_chain_hash.expected_hash);
  try {
    ...
    for (const auto &wallet : eligible_wallets) {
      auto block =
          std::make_shared<PbftBlock>(prev_blk_hash, anchor_hash, order_hash, final_chain_hash, propose_period,
...
std::optional<std::pair<std::shared_ptr<PbftBlock>, std::shared_ptr<PbftVote>>> PbftManager::identifyLeaderBlock(
  ...
  rust::Vec<rustaxa::PbftManagerLeaderCandidateFact> candidate_facts;
  ...
  for (auto &&vote : propose_votes) {
    rustaxa::PbftManagerLeaderCandidateFact fact;
    ...
    if (!weight.has_value() || *weight == 0) {
      fact.weight = 0;
      fact.status = kPbftManagerLeaderCandidateInvalidVoteWeight;
    ...
    if (proposed_block_hash == kNullBlockHash) {
      ...
      fact.status = kPbftManagerLeaderCandidateNullVoteBlockHash;
      candidate_facts.push_back(fact);
      continue;
    }

    if (pbft_chain_->findPbftBlockInChain(proposed_block_hash)) {
      fact.status = kPbftManagerLeaderCandidateBlockInChain;
      candidate_facts.push_back(fact);
      continue;
    }

    auto leader_block = getValidPbftProposedBlock(propose_blocks, vote->getPeriod(), proposed_block_hash);
    if (!leader_block) {
      LOG(log_er_) << "Unable to get proposed block " << proposed_block_hash;
      fact.status = kPbftManagerLeaderCandidateBlockMissingOrInvalid;
      candidate_facts.push_back(fact);
      continue;
    }

    fact.status = kPbftManagerLeaderCandidateReady;
    fact.pivot_hash = toBridgeHash(leader_block->getPivotDagBlockHash());
    materialized_candidates.emplace_back(leader_block, vote);
    candidate_facts.push_back(fact);
  }

  const auto plan = rustaxa::plan_pbft_manager_leader_selection(std::move(candidate_facts));
  if (!plan.selected) {
    return {};
  }
          ...
          if (resume_runtime_session->pbft_finalization_runtime_session_next().action ==
            ...
            try {
              ...
              executed_status_result = rustaxa::apply_pbft_finalization_storage_writes(
                  db_->rustStorage(), finalization_plan.storage_write_intent, std::move(executed_status_stages), false);
            } catch (const std::exception &e) {
...

  {
    ...
    try {
      primary_storage_result = rustaxa::apply_pbft_finalization_storage_writes(
          db_->rustStorage(), finalization_plan.storage_write_intent, std::move(first_persistence_stages), false);
    } catch (const std::exception &e) {
    ...
    if (finalization_plan.cleanup.update_pbft_chain) {
      ...
      const auto pbft_chain_report =
          pbft_chain_->updatePbftChainForPbftFinalization(pbft_block_hash, anchor_hash,
                                                          finalization_plan.storage_write_intent);
  ...
  if (finalization_plan.storage_write_intent.apply_dynamic_lambda_update) {
    ...
    try {
      ...
      dynamic_lambda_result = rustaxa::apply_pbft_finalization_storage_writes(
          db_->rustStorage(), finalization_plan.storage_write_intent, std::move(dynamic_lambda_stages), false);
    } catch (const std::exception &e) {
