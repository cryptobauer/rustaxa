...
The first PBFT-manager application-root cutover is also implemented. Rust mode no longer constructs, exposes, or links
the `PbftManager` facade or `pbft_manager_shim`: `App` starts and stops its single `ConsensusApplication`, daemon and
sync/proposal/certificate/finalization progression enter root tasks, tarcap uses the network API, and public readers use
query/live-status APIs. Local generated-vote admission and own-vote persistence are one native transaction; canonical
proposal bytes are decoded and published under native authority. Pure-C++ mode retains the untouched upstream manager.
`CRW-12` and `CRW-16` remain active because `ConsensusApplication::Runtime` is still a private C++ compatibility
executor for timer, signing, transport, lifecycle, and concrete EVM/FinalChain leaves and still consumes manager-shaped
CXX task/effect carriers. Those carriers and remaining materializers must be replaced by exact leaf ports before the
manager family is fully closed. This checkpoint lowers the checked surface to 20,394 bridge lines, 7,713 shim lines,
347 CXX functions, 288 carriers, 15 handles, six shim directories, and 35 non-test bridge consumers. The coherent live
status fields reuse the existing root runtime snapshot; the obsolete split own-vote persistence export and result
carrier are deleted, so the cut adds no net CXX function and lowers the carrier count.
Terminal `App` teardown also releases the private runtime to break its temporary strong ownership cycle with injected
services, while restartable consensus stop/start remains available during normal operation. Escaped roots reject host
operations after shutdown, and network packet decisions reuse one coherent live-status snapshot per operation.
...
| PBFT manager | `pbft/pbft_manager.hpp`, `pbft/pbft_manager.cpp`, `consensus_application_runtime.*` | 3267 legacy lines plus private executor | `partial`; no Rust-mode manager facade | Native PBFT application service plus private C++ leaf executor | Rust-enabled builds exclude the untouched upstream manager and contain no `PbftManager` object or shim. `App` owns one `ConsensusApplication`; its native `PbftService` owns manager scalar state, daemon/action/session cursors, period-data metadata, sync admission, proposal planning, block validation, transition persistence, PBFT chain lifetime/state, proposed-block state, verified-vote state, atomic local-vote/own-row persistence, and coherent live status. Network and public readers use root network/query APIs. The private `ConsensusApplication::Runtime` still consumes manager-shaped task/effect carriers and materializes temporary objects while executing timer, signing, transport, lifecycle, and concrete FinalChain/EVM leaves; replacing that executor with exact leaf ports remains active `CRW-15`/`CRW-16` debt. Pure-C++ builds retain the untouched original manager. |
...
The next coordinated `CRW-12`/`CRW-14`/`CRW-15`/`CRW-16` vertical cut retires the Rust-mode `PbftChain` facade as a
...
The next `CRW-12`/`CRW-15`/`CRW-16`/`CRW-N01` vertical cut deletes the Rust-mode `VoteManager` facade and its complete
...
granular flags/factories/compatibility constructors, and 36 non-test C++ bridge consumers. One explicit follow-up debt
remains: local vote admission and own-vote persistence are two root calls rather than one atomic application task;
`CRW-12` and `CRW-15` therefore remain active. Native network admission and vote behavior are covered in Rust, while
...
Rust-enabled `StateAPITest.slashing` now generates conflicting canonical votes through the application root, admits them
through `ConsensusNetworkApi`, executes the named C++ signing/insertion leaf, and observes the eventual FinalChain jail.
