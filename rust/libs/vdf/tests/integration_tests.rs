...
use rustaxa_vdf::vdf::{WesolowskiVdf, Solution};
use rustaxa_vdf::prover::{WesolowskiProver, CancellationToken};
...
fn test_multiple_vdf_instances() {
    ...
    for (lambda, time_bits, input, modulus) in test_cases {
        ...
        if time_bits <= 10 {
            ...
            let solution = prover.prove(&token);
            let is_valid = verifier.verify(&solution);
            assert!(is_valid, "Solution should be valid for lambda={}, time_bits={}", lambda, time_bits);
        }
    }
}
...
fn test_caching_behavior() {
    ...
    assert!(warm_duration < cold_duration * 2, "Cached creation should be faster");
    assert!(warm_duration2 < cold_duration * 2, "Cached creation should be faster");
}
...
fn test_memory_usage_many_instances() {
    ...
    let instances: Vec<_> = (0..100).map(|i| {
        let lambda = 64 + (i % 64);
    ...
    for (i, vdf) in instances.iter().enumerate() {
        assert!(*vdf.iterations() > Integer::from(0), "Instance {} should have positive iterations", i);
        assert!(*vdf.modulus() > Integer::from(0), "Instance {} should have positive modulus", i);

        // Test hash-to-prime for a sample of instances
        if i % 10 == 0 {
            let test_input = Integer::from(i);
            let result = vdf.hash_to_prime(&test_input);
            assert!(result.is_ok(), "Hash-to-prime should work for instance {}", i);
        }
    }
    ...
    for (lambda, time_bits, input, modulus, description) in boundary_cases {
        ...
        match result {
            ...
            Err(msg) => {
                ...
                assert!(msg.contains("Prime not found"),
