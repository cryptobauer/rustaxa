...
| `rust/crates/rustaxa-bridge/src/pbft_manager.rs` | `BridgePbftManagerRuntime`, proposal/validation session handles | `pbft_manager_shim`, app bootstrap runtime creation | Internal Rust route | Runtime and state-action effect session handles are retired and owned by `BridgePbftManagerRuntime`; keep folding the remaining proposal and validation sessions into native Rust consensus runtime. Keep only app bootstrap handle until PBFT manager C++ facade is retired. |
