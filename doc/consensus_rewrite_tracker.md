...
| Transaction queue | `transaction/transaction_queue.hpp`, `transaction/transaction_queue.cpp`, `transaction_queue_shim/*` | 501 lines | `rust-backed` | Rust metadata and payload domain behind C++ overlay shim | Deterministic queue metadata, per-account nonce ordering, same-nonce replacement, non-proposer expiry planning, finalized-account purge planning, pool limits, gas-price threshold accounting, queued transaction RLP payload retention, known-cache expiration, and overflow/drop observation state route through Rust under `RUSTAXA_ENABLE_TRANSACTION_QUEUE`. C++ materializes `Transaction` objects from Rust-retained RLP; TransactionManager finalized-account purge now sources account nonce facts from Rust FinalChain. |
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp`, `transaction_manager_shim/*` | 837 lines | `partial` | Rust-owned live sidecars plus C++ materialization/orchestration shell | `packTrxs` candidate snapshotting, candidate scan, declared-gas fit checks, invalid-estimate demotion mutation, accepted output ordering, accepted gas accounting, and stop decisions route through a Rust runtime pack session under `RUSTAXA_ENABLE`; C++ now materializes only the current candidate for gas estimation and final accepted RLP outputs returned by Rust. The Rust-mode shim owns an opaque Rust runtime handle for authoritative transaction count, live queue metadata/payloads, known-cache state, and live non-finalized/recently-finalized transaction sidecars. `isTransactionKnown` now routes through Rust using queue-known facts plus Rust sidecar membership, and `insertTransaction` now uses a staged Rust runtime admission boundary for known-fast-path precheck, verified admission execution, public status mapping, and finalized-location finish mapping instead of shim-local queue-status inference. `saveTransactionsFromDagBlock` sends transaction/account facts to Rust; Rust owns sidecar membership checks, duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, target `TrxCount` planning, the accepted transaction storage batch, accepted non-finalized sidecar insertion, runtime count update, and accepted queue erasure before C++ logs side effects. `getTransaction`, `getTransactions`, and `getBlockTransactions` now check the live C++ pool and Rust sidecar payloads first, then ask Rust storage to resolve pending, finalized regular, finalized system, or missing transaction payloads before C++ materializes objects and applies proposal-period nonce filtering for finalized storage hits. `recoverNonfinalizedTransactions` asks Rust storage for hash/RLP recovery entries and stale-finalized classification; Rust removes stale pending rows before C++ validates hash/RLP consistency and inserts survivor payloads into Rust sidecars. `updateFinalizedTransactionsStatus` sends finalized hashes/RLP payloads to Rust; Rust plans count increments, stale recently-finalized period eviction, periodic queue cleanup, recently-finalized sidecar insertion, non-finalized sidecar removal, known-cache marking, queue erasure, and runtime count update while persisting `TrxCount` before C++ logs side effects. Block-finalized queue expiry and finalized-account purge now use a fused Rust runtime cleanup API that returns explicit per-phase removed hash groups; finalized-account purge facts are sourced from the Rust FinalChain handle instead of C++ `FinalChain::getAccount`. C++ still owns locks, transaction materialization, event emission mechanics, logging, gas estimation/cache, admission account facts, finalized-location fact reads, and broader lifecycle/finalization orchestration. |
...
| Route DagBlockProposer policy through Rust/shim | `partial` | Full overlay shim landed. Rust handles proposer eligibility status decisions, legacy VRF input construction, and deterministic tip selection. Transaction packing decisions now route through the Rust-backed TransactionManager planner before proposer sharding. Remaining gaps: VDF compute payload assembly, block construction/signing, add-block wiring, and lifecycle/network orchestration still live in C++. |
| Route TransactionQueue metadata and payloads through Rust/shim | `rust-backed` | Full overlay shim landed under `RUSTAXA_ENABLE_TRANSACTION_QUEUE`. Rust owns deterministic queue metadata/order/limit behavior, canonical queued transaction RLP payloads, known-cache expiration, overflow/drop observation state, and finalized-account purge planning; C++ materializes `Transaction` objects from Rust-retained bytes and owns FinalChain account reads for purge fact sourcing. |
| Route TransactionManager packing, storage reads, and DAG transaction persistence through Rust/shim | `partial` | Rust runtime pack session landed for `packTrxs` candidate snapshotting, candidate scan, declared-gas fit, invalid-estimate demotion mutation, accepted output ordering, accepted gas accumulation, and stop rules. C++ now materializes only the current candidate for gas estimation and final accepted RLP outputs returned by Rust. The shim now owns a Rust `TransactionManagerRuntime` handle for live queue metadata/payloads, known-cache state, non-finalized and recently-finalized sidecars, and the authoritative transaction count. `saveTransactionsFromDagBlock` now collects transaction/account facts in C++, then calls Rust to perform sidecar membership checks, duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, count planning, accepted storage batching, accepted non-finalized sidecar insertion, and accepted queue erasure. `getTransaction`, `getTransactions`, `getBlockTransactions`, `getNonfinalizedTrx`, and `getNonFinalizedTransaction` materialize Rust sidecar RLP hits before Rust storage misses are resolved with explicit pending/finalized-regular/finalized-system/missing source classification. `recoverNonfinalizedTransactions` now loads hash/RLP recovery entries through Rust, removes stale finalized pending rows in Rust storage, validates survivor payloads in C++, and restores survivor membership into Rust sidecars. `updateFinalizedTransactionsStatus` now sends finalized hash/RLP payloads to Rust; Rust plans count increments, stale recently-finalized eviction, periodic queue cleanup, current-period recently-finalized sidecar insertion, non-finalized sidecar removal, known-cache marking, and queue erasure while persisting `TrxCount` before C++ logs side effects. Block-finalized queue expiry and finalized-account purge now share a fused Rust runtime cleanup API with explicit per-phase removed hash groups, and periodic finalized-account purge now reads sender account nonces from Rust FinalChain before mutating the Rust queue. `excludeFinalizedTransactions` and `verifyTransactionsNotFinalized` now use Rust-owned sidecar membership instead of C++ map facts. `insertTransaction` and `insertValidatedTransaction` now share the Rust runtime admission execution boundary: Rust prechecks known state, performs validated proposable/non-proposable queue mutation, returns event/log intent, requests finalized-location lookup only when needed, and finishes public status mapping from C++ supplied storage facts. The Rust-mode facade owns the public `transaction_added_` event surface and emits it from shim-owned code after Rust accepts a proposable admission. C++ still owns signature recovery, transaction materialization, event dispatch mechanics, admission account reads, FinalChain finalized-location reads, `estimateTransactionGas`, estimation cache, and broader lifecycle/finalization orchestration. Remaining gap: move admission account fact sourcing and estimation/lifecycle orchestration into Rust. |
...
| Route PillarVotes aggregation through Rust/shim | `rust-backed` | Landed under `RUSTAXA_ENABLE_PILLAR_VOTES`; Rust owns period initialization, uniqueness, weighted block aggregation, threshold subset selection, cleanup, Rust-side pillar-vote inspection, and stateless PBFT sync bundle planning. C++ owns live `PillarVote` sidecars keyed by Rust-selected hashes and throws if a Rust-selected vote is missing locally. Direct `PillarVotesOld` vs Rust shim parity coverage now exercises period initialization, duplicate idempotence, same-voter conflict rejection, distinct-weight threshold selection, cleanup, and a planned-insertion path proving the Rust-recovered voter controls uniqueness instead of C++ sidecar recovery. The PBFT sync precheck now rejects period/block mismatches, failed C++ prevalidation facts, same-voter conflicts, zero weights, duplicate-only recounts below threshold, and weight overflow before planned insertion. Signing and full `PillarChainManager` orchestration remain future work. |
| Route PBFT manager pillar-vote bundle validation through Rust | `partial` | The Rust-enabled sync path now enters a shim-owned helper through a narrow early-return hook. The helper inspects each vote through Rust, uses Rust-returned period/block/vote hash/voter/signature status to build bundle facts, looks up DPoS weight in C++ with the Rust-recovered voter, calls the Rust bundle planner, returns explicit shim/planner statuses plus the first bad vote hash, and inserts only Rust-accepted votes with Rust-planned weights and Rust-recovered voter identities through a temporary PillarChainManager hook. This removes `validatePillarVote` / `addVerifiedPillarVote` revalidation, C++ signature recovery, C++ voter recovery, and DPoS relookup from insertion while keeping live sidecar insertion visible as temporary manager debt. Direct CXX bridge coverage for `pillar_vote_inspect` and `plan_pillar_vote_bundle` lives in `rust_consensus_tests`. Remaining PBFT manager state machine, proposal handling, finalization, and gossip orchestration are still C++ owned. |
| Route PillarChainManager relevance through Rust | `partial` | `isRelevantPillarVote` now has a guarded Rust-mode early return that calls a shim-owned helper and stateless Rust relevance planner. Rust owns the deterministic period/block/known-vote decision and stable rejection statuses; C++ still owns current-block lookup, live vote sidecars, logging, network, storage, validation, signing, DPoS queries, and finalization. Replace the guarded hook with a full overlay once the manager boundary is ready. |
| Route PillarChainManager vote validation through Rust inspection | `partial` | `validatePillarVote` now has a guarded Rust-mode path that inspects pillar-vote RLP/signature through Rust, uses the Rust-recovered `(period, vote_hash, voter)` identity for Rust-backed uniqueness checks, uses the recovered voter for C++ FinalChain DPoS eligibility, and returns explicit shim statuses for malformed votes, invalid signatures, conflicts, ineligible validators, and future DPoS state. `addVerifiedPillarVote` is now routed the same way for insertion: inspection-backed recovered identity, recovered-voter DPoS weight lookup, threshold initialization as before, and recovered-voter insertion into `PillarVotes`. C++ still owns logging, FinalChain calls, live sidecars, signing, network, storage, and finalization. C++/Rust bridge coverage verifies Rust recovery matches C++ for a signed vote and rejects out-of-range recovery IDs. |
| Restore `pbft_manager.cpp` upstream shape after complete shim | `partial` | The current file diff is reduced to a guarded early-return hook, with helper declarations supplied by a temporary `pbft_manager.hpp` shim overlay. Treat that touch point as merge-conflict debt: once a full `PbftManager` shim owns Rust-mode routing, move the hook into the shim class and revert `libraries/core_libs/consensus/src/pbft/pbft_manager.cpp` back to upstream shape. |
