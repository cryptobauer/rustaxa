...
| `slashing_manager_shim` | Standalone slashing proof planner facade; Rust vote admission passes one `SlashingDoubleVoteEvidence` payload while the live `PbftVote` overload is only a compatibility adapter | Slashing manager users | C++ public compatibility facade | The facade no longer imports or compiles `SlashingManagerOld`; its module flag remains valid for partial configurations. Delete after slashing planning and transaction submission run inside Rust consensus/runtime ports. The missing pre-Magnolia submission gate is tracked as a separate planner-parity gap. |
