...
| --- | --- | --- | --- | --- |
| `rust/crates/rustaxa-bridge/src/dag.rs` | DAG query, storage, finalization, and carrier-conversion helpers over native `DagService` state | DAG manager shim | Internal bridge route | Move remaining manager query and compatibility tasks native, leaving only unavoidable CXX carrier conversion. |
