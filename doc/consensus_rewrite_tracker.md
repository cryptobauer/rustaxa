...
| Route TransactionQueue metadata and payloads through Rust/shim | `rust-backed` | Full overlay shim landed under `RUSTAXA_ENABLE_TRANSACTION_QUEUE`. Rust owns deterministic queue metadata/order/limit behavior, canonical queued transaction RLP payloads, known-cache expiration, and overflow/drop observation state; C++ materializes `Transaction` objects from Rust-retained bytes and owns FinalChain account reads for purge. |
