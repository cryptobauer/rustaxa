...
Rust-owned period/index state rather than querying the old manager. Ordered non-finalized sync block selection,
non-finalized transaction query planning, and expired-block transaction cleanup selection also route through Rust hash
plans while C++ still owns live transaction objects and transaction-manager side effects.
