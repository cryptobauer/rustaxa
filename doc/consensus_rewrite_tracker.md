...
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp`, `transaction_manager_shim/*` | 837 lines | `partial` | C++ shell plus Rust packing/storage bridge | `packTrxs` candidate sizing, declared-gas fit checks, invalid-estimate demotion decisions, accepted gas accounting, and stop decisions route through `rustaxa-consensus::transaction_manager` under `RUSTAXA_ENABLE`. DAG-accepted transaction persistence now batches accepted transaction RLP payloads and the manager-owned `TrxCount` through Rust storage before C++ updates live non-finalized/pool caches. C++ still owns live transactions, locks, gas estimation/cache, queue mutation, FinalChain, and broader lifecycle/finalization state. |
...
| Route TransactionManager packing and DAG transaction persistence through Rust/shim | `partial` | Rust planner landed for `packTrxs` candidate cap, declared-gas fit, invalid-estimate demotion, accepted gas accumulation, and stop rules. `saveTransactionsFromDagBlock` now selects accepted transactions with C++ live caches/FinalChain reads, persists their RLP payloads plus the target `TrxCount` through a dedicated Rust storage batch, and only then mutates C++ non-finalized/pool state. C++ still owns live transaction materialization, `estimateTransactionGas`, estimation cache, queue mutation, and broader lifecycle/finalization state. |
