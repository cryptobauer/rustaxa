...
| Pillar block/votes | `pillar_chain/pillar_block.hpp`, `pillar_chain/pillar_votes.hpp`, matching `.cpp` files | 627 lines | `rust-domain` for pillar type/codecs | Rust domain and codecs | `rustaxa-types::pillar` now mirrors `PillarBlock`, `ValidatorVoteCountChange`, `PillarVote`, `PillarBlockData`, optimized pillar-vote bundle, and current pillar data RLP/Solidity/hash shapes with malformed-input coverage and C++ golden-vector coverage for Solidity bytes. Vote signing/recovery, manager orchestration, JSON/RPC, and production routing are not started. |
