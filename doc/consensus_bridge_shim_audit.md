...
| `rust/crates/rustaxa-bridge/src/pbft_chain.rs` | `BridgePbftChain`, `create_pbft_chain_from_storage` | `pbft_chain_shim`, PBFT manager/runtime tests | C++ public compatibility facade | The storage-backed handle clones and retains its own `Arc<Storage>` and the standalone facade no longer imports or compiles `PbftChainOld`. Delete once the public C++ facade is no longer required or PBFT manager owns chain state natively in Rust. |
...
| `BridgePbftChain` | `pbft_chain.rs` | `pbft_chain_shim`, PBFT tests | C++ public compatibility facade | `CRW-01` selected one PBFT application service as the owner of chain and manager state. In `CRW-02`, delete this independently constructed handle and rehome its read/materialization methods on the shared service; retain the public C++ `PbftChain` facade as a narrow view for network, DAG, vote, RPC, stats, and tests. |
| `BridgePbftManagerRuntime` | `pbft_manager.rs` | App bootstrap, `pbft_manager_shim` | Internal Rust route | `CRW-01` selected one PBFT application service as the owner of chain and manager state. In `CRW-02`, delete this independently constructed handle after its runtime/session methods move to the service and app bootstrap creates only the shared service. |
...
| `pbft_chain_shim` | Standalone Rust-backed PBFT chain facade | PBFT manager, network/tarcap, DAG/vote compatibility, RPC/public readers, stats, and tests | C++ public compatibility facade | `CRW-02` moves chain state and finalization mutation authority into the shared PBFT service. Keep this C++ class primarily as a read, JsonCpp, and `PbftBlock` materialization view; replace `App`'s production `DbStorage` construction path and independent Rust handle first. Preserve the public `PbftChain(addr_t, std::shared_ptr<DbStorage>)` and `updatePbftChain(...)` signatures as classified compatibility/test adapters over service-owned state until their direct callers migrate. |
| `pbft_manager_shim` | Standalone PBFT manager Rust runtime facade; feature-on builds import and compile no legacy manager scaffold | App bootstrap and consensus loop | Internal Rust route | `CRW-02` replaces its independent manager-runtime handle with the shared PBFT service, internalizes manager-to-chain reads and finalization mutation, and leaves only classified lifecycle/executor/materialization shell work. |
