...
| Route TransactionManager packing and DAG transaction persistence through Rust/shim | `partial` | Rust planner landed for `packTrxs` candidate cap, declared-gas fit, invalid-estimate demotion, accepted gas accumulation, and stop rules. `saveTransactionsFromDagBlock` now collects only live transaction/cache/account facts in C++, then calls Rust to perform duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, count planning, and the accepted transaction storage batch. `updateFinalizedTransactionsStatus` now collects finalized transaction hash/cache facts in C++, then calls Rust to plan count increments, stale recently-finalized period eviction, and periodic queue purge while persisting `TrxCount` before C++ mutates live recently-finalized, known-cache, non-finalized, and pool sidecars. C++ mutates non-finalized/pool sidecars only for Rust-returned accepted input indexes. C++ still owns live transaction materialization, `estimateTransactionGas`, estimation cache, queue mutation, and broader lifecycle/finalization state. |
