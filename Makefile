...
REWRITE_CONSENSUS_TESTS?=rust_consensus_tests dag_test dag_block_test pbft_manager_test vote_test pillar_chain_test
...
.PHONY: rewrite-bridge-inventory-guard
rewrite-bridge-inventory-guard: ## Require bridge CXX handles to be documented in the audit.
...
rewrite-validate-consensus: rewrite-validate-fast ## Run Rust validation plus targeted consensus C++ tests.
	...
	@"$(BUILD_OUTPUT_DIR)/bin/network_test" --gtest_filter=NetworkTest.rust_mode_pbft_sync_via_query_client
