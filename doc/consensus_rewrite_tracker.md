...
Current selection: continue `CRW-04` after the transaction/gas composition slice embedded the native gas-price
...
untouched original implementation for direct `Dag`/`PivotTree` reference tests. Rust-enabled builds keep a Rust-owned
`DagManagerRuntime` for deterministic in-memory state and storage-backed manager decisions. Frontier, ghost path,
