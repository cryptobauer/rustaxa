...
| Slashing proof planning | Rust validation plus `slashing_manager_shim_test`; add parity fixtures when richer legacy vote/submission transcripts are available |
...
| Route SlashingManager proof planning through Rust/shim | `partial` | Rust planner landed under `RUSTAXA_ENABLE_SLASHING_MANAGER` for report flag gating, PBFT slot equality, canonical double-vote proof hash/cache, submitter selection, contract envelope, and calldata construction. C++ still owns vote object/RLP sourcing, FinalChain account reads, gas bid, transaction signing, and insertion; duplicate cache marking occurs only after successful transaction insertion. |
