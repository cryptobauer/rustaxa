...
| `rust/crates/rustaxa-bridge/src/pillar_votes.rs` | `BridgePillarVotes`, `create_pillar_votes_index`, batch RLP inspection and weighted apply helper | `pillar_chain_manager_shim` (direct in-memory aggregation and sync bundle apply), period-data/vote paths | Internal Rust route | The C++ `pillar_votes_shim` facade is retired. Keep this bridge surface only while `pillar_chain_manager_shim` owns vote indexing and external FinalChain DPoS reads. Delete the batch RLP helpers once a native pillar-chain runtime owns bundle validation and weight lookup ports directly. |
