...
| Route TransactionQueue metadata and payloads through Rust/shim | `rust-backed` | Full overlay shim landed under `RUSTAXA_ENABLE_TRANSACTION_QUEUE`. Rust owns deterministic queue metadata/order/limit behavior and canonical queued transaction RLP payloads; C++ materializes `Transaction` objects from Rust-retained bytes and owns known-cache expiration, overflow timing, and FinalChain account reads for purge. |
