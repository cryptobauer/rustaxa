...
| `rust/crates/rustaxa-bridge/src/sortition.rs` | Sortition methods on `BridgeDagTransactionService` | `sortition_params_manager_shim`, DAG proposer, and the PBFT cross-service finalization operation | Application-service state plus C++ compatibility facade | The standalone handle and factories are deleted. PBFT finalization atomically publishes sortition state through the two application-owned services, and DAG verification loads historical parameters inside the composed DAG service; both C++ report/fact bounces are deleted. Keep the facade methods for public compatibility and proposer callers until their remaining relays move native. |
