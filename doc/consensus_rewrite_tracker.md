...
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp`, `transaction_manager_shim/*` | 837 lines | `partial` | Rust-owned live sidecars plus C++ materialization/orchestration shell | `packTrxs` candidate sizing, declared-gas fit checks, invalid-estimate demotion decisions, accepted gas accounting, and stop decisions route through `rustaxa-consensus::transaction_manager` under `RUSTAXA_ENABLE`; invalid-estimate demotion now mutates Rust queue state directly instead of erase/reinsert through C++. The Rust-mode shim owns an opaque Rust sidecar handle for authoritative transaction count, live non-finalized/recently-finalized transaction membership, and canonical RLP payloads. `isTransactionKnown` now routes through Rust using queue-known facts plus Rust sidecar membership, and known-hash `insertTransaction` outcomes now route through the Rust insert planner instead of a shim-local early return. `saveTransactionsFromDagBlock` sends transaction/account facts to Rust; Rust owns sidecar membership checks, duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, target `TrxCount` planning, the accepted transaction storage batch, accepted non-finalized sidecar insertion, and runtime count update before C++ updates only pool side effects. `getTransaction`, `getTransactions`, and `getBlockTransactions` now check the live C++ pool and Rust sidecar payloads first, then ask Rust storage to resolve pending, finalized regular, finalized system, or missing transaction payloads before C++ materializes objects and applies proposal-period nonce filtering for finalized storage hits. `recoverNonfinalizedTransactions` asks Rust storage for hash/RLP recovery entries and stale-finalized classification; Rust removes stale pending rows before C++ validates hash/RLP consistency and inserts survivor payloads into Rust sidecars. `updateFinalizedTransactionsStatus` sends finalized hashes/RLP payloads to Rust; Rust plans count increments, stale recently-finalized period eviction, periodic queue purge, recently-finalized sidecar insertion, non-finalized sidecar removal, and runtime count update while persisting `TrxCount` before C++ mutates known-cache and pool side effects. C++ still owns locks, transaction materialization, pool/known-cache side effects, gas estimation/cache, FinalChain account fact reads, and broader lifecycle/finalization orchestration. |
...
| Route TransactionManager packing, storage reads, and DAG transaction persistence through Rust/shim | `partial` | Rust planner landed for `packTrxs` candidate cap, declared-gas fit, invalid-estimate demotion, accepted gas accumulation, and stop rules. Invalid-estimate demotion now mutates Rust queue state directly rather than rebuilding queue entries through C++. The shim now owns a Rust `TransactionManagerSidecar` handle for non-finalized and recently-finalized membership plus canonical RLP payloads. `saveTransactionsFromDagBlock` now collects transaction/account facts in C++, then calls Rust to perform sidecar membership checks, duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, count planning, accepted storage batching, and accepted non-finalized sidecar insertion. `getTransaction`, `getTransactions`, `getBlockTransactions`, `getNonfinalizedTrx`, and `getNonFinalizedTransaction` materialize Rust sidecar RLP hits before Rust storage misses are resolved with explicit pending/finalized-regular/finalized-system/missing source classification. `recoverNonfinalizedTransactions` now loads hash/RLP recovery entries through Rust, removes stale finalized pending rows in Rust storage, validates survivor payloads in C++, and restores survivor membership into Rust sidecars. `updateFinalizedTransactionsStatus` now sends finalized hash/RLP payloads to Rust; Rust plans count increments, stale recently-finalized eviction, periodic queue purge, current-period recently-finalized sidecar insertion, and non-finalized sidecar removal while persisting `TrxCount` before C++ mutates known-cache and pool side effects. `excludeFinalizedTransactions`, `verifyTransactionsNotFinalized`, and `insertValidatedTransaction` now use Rust-owned sidecar membership instead of C++ map facts, and Rust now returns explicit admission/finalization side-effect actions so queue, known-cache, and pool mutations execute from Rust-planned intent rather than shim-local inference. The Rust-mode facade owns the public `transaction_added_` event surface and emits it from shim-owned code for Rust-planned proposable admissions before live queue insertion, matching legacy timing. C++ still owns signature recovery, transaction materialization, pool/known-cache timing, FinalChain reads, `estimateTransactionGas`, estimation cache, and broader lifecycle/finalization state. Remaining gap: move transaction pool/known-cache side effects and broader lifecycle orchestration out of the shim. |
