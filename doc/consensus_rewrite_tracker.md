...
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp`, `transaction_manager_shim/*` | 837 lines | `partial` | C++ live sidecar plus Rust packing/persistence/finalized-status/storage-read planner | `packTrxs` candidate sizing, declared-gas fit checks, invalid-estimate demotion decisions, accepted gas accounting, and stop decisions route through `rustaxa-consensus::transaction_manager` under `RUSTAXA_ENABLE`; invalid-estimate demotion now mutates Rust queue state directly instead of erase/reinsert through C++. `saveTransactionsFromDagBlock` now sends live transaction facts to Rust; Rust owns duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, target `TrxCount` planning, and the accepted transaction storage batch before C++ updates only accepted live non-finalized/pool sidecars. `getTransaction`, `getTransactions`, and `getBlockTransactions` now check live C++ sidecars or Rust queue payloads first, then ask Rust storage to resolve pending, finalized regular, finalized system, or missing transaction payloads before C++ materializes objects and applies proposal-period nonce filtering for finalized storage hits. `recoverNonfinalizedTransactions` now asks Rust storage for hash/RLP recovery entries and stale-finalized classification; Rust removes stale pending rows before C++ validates hash/RLP consistency and rebuilds only survivor sidecars. `updateFinalizedTransactionsStatus` now sends finalized hashes and live non-finalized-cache facts to Rust; Rust plans count increments, stale recently-finalized period eviction, and periodic queue purge while persisting `TrxCount` before C++ mutates live maps/cache/pool sidecars. C++ still owns locks, manager non-finalized/recently-finalized sidecars, gas estimation/cache, FinalChain account fact reads, and broader lifecycle/finalization state. |
...
| Route TransactionManager packing, storage reads, and DAG transaction persistence through Rust/shim | `partial` | Rust planner landed for `packTrxs` candidate cap, declared-gas fit, invalid-estimate demotion, accepted gas accumulation, and stop rules. Invalid-estimate demotion now mutates Rust queue state directly rather than rebuilding queue entries through C++. `saveTransactionsFromDagBlock` now collects only live transaction/cache/account facts in C++, then calls Rust to perform duplicate filtering, nonce-gated finalized-storage lookup, accepted ordering, count planning, and the accepted transaction storage batch. `getTransaction`, `getTransactions`, and `getBlockTransactions` are shim-owned: C++ checks live non-finalized/recently-finalized sidecars and Rust queue payloads first, Rust resolves storage misses with explicit pending/finalized-regular/finalized-system/missing source classification, and C++ materializes `Transaction`/`SystemTransaction` objects while preserving proposal-period nonce filtering for finalized storage hits. `recoverNonfinalizedTransactions` now loads hash/RLP recovery entries through Rust, removes stale finalized pending rows in Rust storage, and mutates C++ live sidecars only after survivor materialization and hash validation succeeds. `updateFinalizedTransactionsStatus` now collects finalized transaction hash/cache facts in C++, then calls Rust to plan count increments, stale recently-finalized period eviction, and periodic queue purge while persisting `TrxCount` before C++ mutates live recently-finalized, known-cache, non-finalized, and pool sidecars. `excludeFinalizedTransactions` and `verifyTransactionsNotFinalized` now route recently-finalized and nonce facts through Rust-backed finalized-storage checks and deterministic plans. `verifyTransaction`, `insertTransaction`, and `insertValidatedTransaction` now route transaction/config/cache/account facts through Rust planners for exact legacy verification statuses, public insert result mapping, and pre-queue proposable/non-proposable admission. The Rust-mode facade owns the public `transaction_added_` event surface and emits it from shim-owned code for Rust-planned proposable admissions before live queue insertion, matching legacy timing. `getNonfinalizedTrx`, `getNonFinalizedTransaction`, `getTransactionCount`, pool state reads, and live queue limit/min-gas helpers are shim-owned under the existing mutex instead of forwarding to `TransactionManagerOld`. C++ mutates non-finalized/pool sidecars only for Rust-returned accepted input indexes and still owns signature recovery, known-cache timing, FinalChain reads, `estimateTransactionGas`, estimation cache, and broader lifecycle/finalization state. Remaining gap: move manager sidecars and lifecycle orchestration out of the shim. |
