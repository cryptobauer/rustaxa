...
operation-specific threshold plan. CRW-09I remains active because local sortition and PBFT-manager
validation/eligibility still consume generic `PbftFinalChainFact*` carriers. The validation
...
state-lifecycle, configuration, or public-materialization boundaries, so `CRW-09` is complete and `CRW-10` is ready.
...
| `CRW-E01` | `blocked` | Change the external EVM/StateAPI contract, including concrete state execution, receipt execution details, arbitrary contract calls, `state_db/` mutation, or removal of temporary C++ `rewards::BlockStats` materialization. | Explicitly expand the accepted external-EVM/StateAPI boundary in `PLAN.md`. | A separately approved design and parity plan replaces the affected executor contract; StateAPI consumes a Rust-native/canonical rewards-stat contract without C++ consensus materialization, and external execution ownership remains explicit. |
...
| Pillar manager | `pillar_chain/pillar_chain_manager.hpp`, `pillar_chain/pillar_chain_manager.cpp`, `pillar_chain_manager_shim/*` | 629 lines | `partial` | C++ compatibility/executor facade over PBFT-service-owned pillar state | The App-owned `BridgePbftService` restores and owns the single production pillar state behind an independent readiness gate and sibling mutex, and composes Pillar validator facts directly with borrowed Rust FinalChain state. Production `PillarChainManager` receives that shared service; the standalone runtime handle/factory are deleted. The full overlay retains bridge root/epoch execution, network transport, signing, C++ block/vote materialization, storage-compatible payloads, events, and finalization effects as explicit executor boundaries. A pillar-only partial service is confined to compatibility construction/tests, and module-disabled/pure-C++ builds retain the untouched legacy manager and `PillarVotes` implementations. |
...
| Route DagBlockProposer policy through Rust/shim | `partial` | The standalone overlay facade no longer imports or compiles `DagBlockProposerOld` in feature-on builds. Rust owns proposer eligibility status decisions, legacy VRF input construction, deterministic tip selection, transaction-pack command flow through the Rust-owned TransactionManager pack session, proposal timestamps, VDF input/message bytes, VDF wait/cancel/stale-proof decisions, retry-cursor updates, block construction planning, final signed-RLP construction after temporary C++ node-secret signing, and manager submission through signed RLP plus transaction payloads. The cursor-bound DAG service now snapshots proposal identity, releases its lock, reads FinalChain height and DPoS/VRF authorization directly from the Rust FinalChain runtime, then reacquires and revalidates the cursor and historical sortition parameters; C++ supplies only the session id and shim-private runtime reference. Remaining C++ is classified as executor/compatibility work: proposer worker lifecycle, live network throttle checks, async VDF compute execution, node-secret signature execution, compatibility materialization, add-block effect execution, logging, and network egress. |
...
| Route PBFT manager pillar-vote bundle validation through Rust | `partial` | The Rust-enabled sync path now enters the PBFT-service-owned pillar runtime from the full `PbftManager` overlay. Rust inspects canonical vote RLP, recovers voter identities, binds preparation to the current anchor generation, borrows Rust FinalChain for ordered DPoS weights and the total, and revalidates before weighted insertion and threshold initialization. C++ retains live vote-sidecar resolution and manager dispatch but no longer builds per-vote FinalChain fact payloads or revalidates accepted votes. Native Rust composed tests replace the retired low-level fact-supplied CXX contracts. Broader PBFT manager proposal, transition, and finalization paths are tracked in the dedicated PBFT manager rows; network gossip execution and many live compatibility side effects remain C++ executor work. |
...
| Route PillarChainManager relevance through Rust | `rust-backed` | The Rust-mode path now lives in the full `PillarChainManager` overlay instead of an original-source guarded hook. `isRelevantPillarVote` enters PBFT-service-owned pillar state, so Rust owns current-anchor lookup, deterministic period/block/known-vote decisions, and stable rejection statuses. C++ retains live vote sidecars, network, signing, finalization, and temporary log emission; validation and DPoS composition use the same Rust service. Logging is not an ownership blocker. |
| Route PillarChainManager vote validation and block planning through Rust | `partial` | Pillar state is now a private sibling of PBFT manager/chain/vote/slashing state in `BridgePbftService`, with distinct readiness and exact unavailable behavior for partial services. Rust owns PBFT candidate/current-anchor decisions, strict-majority threshold arithmetic, admission/bundle contexts, validator vote-count history, block creation/linkage, PBFT-facing prepare/acknowledge publication, and the complete Pillar FinalChain DPoS composition family. FinalChain reads run after releasing the pillar mutex; apply reacquires it and verifies the retained preparation or anchor generation. C++ supplies bridge root/epoch executor facts and retains signing, tarcap transport, logging, event dispatch, and temporary `PillarBlock`/`PillarVote`/`PeriodData` materialization. The generic `PbftFinalChainFact*` carriers remain for PBFT manager and VoteManager consumers. |
