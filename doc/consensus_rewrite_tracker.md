...
| Pillar block/votes | `pillar_chain/pillar_block.hpp`, `pillar_chain/pillar_votes.hpp`, matching `.cpp` files | 627 lines | `rust-domain` for pillar block Solidity encoding/hash and optimized vote bundle shape | Rust domain and codecs | `rustaxa-types::pillar` now mirrors `PillarBlock::encodeSolidity()` and `getHash()` for block payloads with C++ golden-vector coverage, and preserves `PillarBlockData`/optimized pillar-votes bundle RLP shape. Full `PillarBlock` RLP, vote signing/recovery, manager orchestration, and production routing are not started. |
