...
Canonical bytes and typed reports cross only those ports; the pillar compatibility facade queries canonical current
pillar bytes directly from the native owner. Manager task/action transcripts, legacy materializers, and their
...
result validation. `CRW-12`, `CRW-14`, `CRW-15`,
and `CRW-16` stay active for other remaining subsystem families, while `CRW-N01` stays active until every remaining
handler-local consensus decision is native.

The checked surface after this cut is 12,100 bridge lines, 4,075 shim lines, 211 CXX functions, 170 carriers, 14 opaque
...
| --- | --- | --- | --- |
| PBFT, vote, pillar | PBFT family private under `CRW-12` root | Affected `CRW-14` facades and `CRW-15` object families | PBFT portion of `CRW-16`; coordinate transport leaves with `CRW-N01` |
...
FinalChain account facts, pillar-anchor-state facts, and concrete EVM execution leaves. The remaining pillar facade
reads canonical current-pillar bytes from the native owner instead of a C++ runtime mirror. Independent review corrected
