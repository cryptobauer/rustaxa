...
| `rust/crates/rustaxa-bridge/src/transaction_manager.rs` | DTO conversion plus short-lived admission, DAG-save/finalized-status, recovery, and finalized filter/verification adapters over native transaction ownership; read, packing, cache, sidecar-removal, and queue-finalization tasks call lock-owning native services directly | transaction/DAG/PBFT shims | Internal bridge route | Move the remaining fact-backed FFI task methods into native service APIs; retain only submission/materialization conversion and unlocked EVM leaf adapters. |
