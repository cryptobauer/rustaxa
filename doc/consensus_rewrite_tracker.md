...
| Transaction manager | `transaction/transaction_manager.hpp`, `transaction/transaction_manager.cpp`, `transaction_manager_shim/*` | 837 lines | `partial` | C++ shell plus Rust packing planner | `packTrxs` candidate sizing, declared-gas fit checks, invalid-estimate demotion decisions, accepted gas accounting, and stop decisions route through `rustaxa-consensus::transaction_manager` under `RUSTAXA_ENABLE`. C++ still owns live transactions, locks, gas estimation/cache, queue mutation, FinalChain, storage, and DAG status transitions. |
...
`verifyBlock` flow directly for prechecks, Rust-planned transaction lookup, DAG VDF payload/difficulty/proof verification,
legacy DAG VRF/VDF message construction, VDF/DPoS authorization decision ordering, and Rust-backed gas policy decisions
instead of forwarding the method wholesale to `DagManagerOld`. Finalized DAG order application also now advances the
...
| Route DagManager verify/finalization state through Rust/shim | `partial` | Tip count/uniqueness, proposal-period availability, expiry, transaction availability, verify-side transaction query planning, DAG embedded-VRF/VDF payload/difficulty/proof verification, legacy DAG VRF/VDF message construction, verify-side VDF max-vote ceiling selection, VDF/DPoS authorization ordering, gas-policy decisions, finalized DAG order application, empty-period advancement, expiry-level advancement, transitive expired-block classification, counter-update planning, non-finalized DAG sync period/index reads, ordered non-finalized sync block selection, sync transaction RLP lookup, non-finalized transaction query planning, and expired-block transaction cleanup selection route through Rust. The shim still owns legacy block/transaction object materialization, cache cleanup, DB side-effect calls, and transaction-manager side effects, while DPoS/VRF facts now come from a Rust FinalChain bridge bundle and feed a single status-coded Rust envelope. Remaining gap: move verify-side live transaction materialization and broader orchestration out of the shim. |
...
| Route TransactionManager packing through Rust/shim | `partial` | Rust planner landed for `packTrxs` candidate cap, declared-gas fit, invalid-estimate demotion, accepted gas accumulation, and stop rules. C++ still owns live transaction materialization, `estimateTransactionGas`, estimation cache, queue mutation, and lifecycle/finalization state. |
