...
| `CRW-03` | `active` | Absorb PBFT-private state handles into the PBFT application service, starting with proposed blocks and verified votes, then rewards/sortition state used only by PBFT progress and finalization. | `CRW-02` | Chain-private state is no longer independently located or passed through CXX; obsolete `BridgeProposedBlocks`, `BridgeVerifiedVotes`, and PBFT-only rewards/sortition exports and shim mechanics are deleted after their callers migrate. |
| `CRW-04` | `ready` | Compose transaction/gas and DAG graph/manager/proposer runtimes behind application-owned Rust services with native FinalChain/storage ports. | `CRW-01`; coordinate shared dependencies with `CRW-02` | C++ shims no longer pass internal bridge handles between transaction, DAG, PBFT, FinalChain, or storage services; they perform input conversion, explicit EVM/network execution, and public materialization only. |
| `CRW-05` | `ready` | Compose pillar, slashing, sortition, and rewards planning/state behind their Rust application owner rather than standalone internal handles. | `CRW-01`; `CRW-02` where PBFT owns the lifetime | Remaining C++ code is limited to FinalChain/DPoS fact execution, signing, transaction insertion, tarcap/event execution, and public materialization; internal bridge handles and cross-shim lookup paths are deleted. |
...
Recommended next selection: implement `CRW-03` so proposed-block and verified-vote state become private to the PBFT
application service before selecting the broader sibling-service work in `CRW-04` or `CRW-05`.
...
`App` owns one shim-local `PbftService` RAII holder and shares it with the retained C++ `PbftChain` and `PbftManager`
facades. The app bootstrap edit is an intentional guarded `RUSTAXA_ENABLE` exception in upstream-owned `app.hpp` and
...
`CRW-03` is active as two dependency-ordered implementation sub-slices. The first moves proposed-block state into the
...
The second sub-slice moves verified-vote/admission state into the same service and deletes `BridgeVerifiedVotes` after
the retained C++ vote/network facades become service clients. Proposed-block absorption lands first because leader
selection currently crosses from vote management into proposed-block state; absorbing votes first would preserve or
recreate that cross-shim dependency. The overall item remains active until the verified-vote and remaining PBFT-private
rewards/sortition deletion conditions are resolved.

The current service uses sibling Rust lock domains for manager, proposed blocks, and chain state. Proposed-block
operations do not hold a guard across C++ validation, network, FinalChain/EVM, logging, or gossip callbacks. The
verified-vote follow-up will add its sibling lock and extend production construction to restore votes before bootstrap
publication; `BridgeVerifiedVotes` remains independently owned until that commit.
...
second process-local index exists. The item remains active for verified votes and subsequent private state.
...
| PBFT manager | `pbft/pbft_manager.hpp`, `pbft/pbft_manager.cpp`, `pbft_manager_shim/*` | 3267 lines | `partial` | PBFT application service plus C++ lifecycle/executor facade | Rust-enabled builds exclude the untouched upstream manager source and expose only the shim-owned `PbftManager` facade. `BridgePbftService` owns manager scalar state, daemon/action/session cursors, period-data metadata, sync admission, proposal planning, block validation, transition persistence, PBFT chain lifetime/state, and proposed-block state. Production construction restores chain and proposed blocks, derives manager period/Cacti activation, and remains bootstrap-gated until C++ replay/restart work completes. Finalization drains and validates the chain mutation internally; C++ executes only the remaining typed external effects for FinalChain/EVM, DAG, network, timers, signing, events, and compatibility materialization. Verified-vote state is the next private-state absorption target under `CRW-03`. |
