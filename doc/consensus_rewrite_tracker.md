...
lookup. Both production and chain-only bridge constructors publish that native owner; the old bridge chain-state struct
and bridge-owned `Arc<RwLock<_>>` are deleted. Cross-domain finalization and leader-selection code temporarily borrow
...
owns its shared mutex. `BridgePbftService` now stores only an optional native service capability for the still-live
chain-only compatibility topology; it no longer owns a `Mutex<Option<PbftVoteAdmissionRuntime>>`. Native service tests
...
| PBFT chain | `pbft/pbft_chain.hpp`, `pbft/pbft_chain.cpp`, `pbft_chain_shim/*` | 259 lines | `rust-owned` application state; C++ facade retained | Native `rustaxa-consensus::pbft_chain::PbftChainService` behind a C++ compatibility view | The CXX-free native owner holds startup restore/default initialization, storage lifetime, the sibling `RwLock`, head projection/update, block lookup, and next-block validation. `BridgePbftService` embeds that owner; production `App` shares it between manager and chain facades, and the stable public `PbftChain(DbStorage)` constructor remains a chain-only compatibility/test adapter. Cross-domain finalization and leader selection temporarily borrow native guards until the complete PBFT owner moves. C++ retains JsonCpp formatting and temporary `PbftBlock` materialization. Feature-on builds import or compile no `PbftChainOld`; pure-C++ builds retain the untouched original implementation. |
| Proposed blocks | `pbft/proposed_blocks.hpp`, `pbft/proposed_blocks.cpp`, `proposed_blocks_shim/*` | 178 lines | `rust-owned` application state; C++ facade retained | Native `rustaxa-consensus::proposed_blocks::ProposedBlocksService` behind C++ materialization facade | The CXX-free native owner holds restore, membership, compact pivot metadata, validation flags, canonical RLP payloads, storage-first publication, atomic stale-period cleanup, storage lifetime, and the sibling `RwLock`; native behavioral tests cover those contracts. `BridgePbftService` embeds the owner and proposed-block bridge methods only convert native results to FFI DTOs. Cross-domain leader selection and combined vote/proposal cleanup temporarily borrow native guards until the complete PBFT owner moves under `CRW-12`. `BridgeProposedBlocks`, its factory, the storage-shim-owned live handle, and the C++ facade mutex are deleted. `DbStorage` compatibility uses stateless storage functions; tentative wallet candidates use an isolated Rust-local batch lookup. The retained facade materializes temporary `PbftBlock` values only pending `CRW-14`. |
...
| Verified votes | `vote_manager/verified_votes.hpp`, `vote_manager/verified_votes.cpp`, `verified_votes_shim/*` | 384 lines | `rust-backed` | Native PBFT verified-vote service behind C++ materialization facade | Unique-voter checks, voted-value weight aggregation, 2t+1 block mappings, period cleanup, and round t+1 markers route to `rustaxa-consensus::verified_votes` through `RUSTAXA_ENABLE_VERIFIED_VOTES`. Native `PbftVerifiedVotesService` owns the storage lifetime, atomic restoration, `PbftVoteAdmissionRuntime`, and shared mutex; production `BridgePbftService` holds only that native capability, while the chain-only compatibility service has none. Storage-free construction and its bridge-only tests are deleted. `BridgeVerifiedVotes`, its factory, the facade-owned box, and the C++ mutex are deleted. C++ reconstructs transient `PbftVote` objects only from owned compatibility results; full state, step buckets, current rewards, mutation outcomes, and authoritative leader candidates are captured under one native vote-lock epoch. Leader validation reports are accepted only after the vote/proposed/chain fingerprint is rebuilt unchanged. Admission checkpoints only its replay delta, touched round, and payload entry; required progress rows commit before publication, and failure restores the checkpoint with all executor effects suppressed. Period cleanup is planned and published atomically with proposed-block cleanup by the service. Get-next-votes egress uses a Rust plan/build API returning ordered vote hashes and optimized bundle RLP bytes. The standalone C++ facade owns exact compatibility carriers but no authoritative state; Rust-enabled builds do not import or compile `VerifiedVotesOld`. |
...
| PBFT chain/proposed-block/queue routing | Rust validation plus `rust_consensus_tests`, `pbft_chain_test`, `pbft_chain_shim_test`, `proposed_blocks_shim_test`, the Rust PBFT service queue tests, and relevant `pbft_manager_test` cases |
